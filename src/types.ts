...
export function getContactAccountCode(contact: Partial<Contact>): string {
  const customCode = String(contact.accountCode || "").trim();
  if (customCode) {
    return customCode;
  }
  ...
  const taxNum = String(contact.taxNumber || "").trim() || "0000000000";
...
export interface Employee {
  ...
  isRetired?: boolean; // Emekli Personel (SGDP - Sosyal Güvenlik Destek Primi tabi)
...
export interface PayrollRecord {
  ...
  isRetired?: boolean; // Emekli Personel (SGDP Kesintileri)
}
...
export type ConstructionCostCategory = "sgk" | "elektrik" | "tesisat" | "kaba_insaat" | "ince_iscilik" | "other";

export interface ProjectCostItem {
  id: string;
  type: ProjectCostType; // Malzeme, İşçilik, Taşeron, Genel Gider, Diğer
  constructionCategory?: ConstructionCostCategory; // SGK Maliyet, Elektrik, Sıhhi Tesisat, Kaba İnşaat, İnce İşçilik
  description: string;
  quantity: number;
  unit: string;
  unitCost: number;
...
export interface CostProject {
  ...
  createdAt: string;
  // İnşaat ve SGK Asgari İşçilik Detayları
  constructionAreaM2?: number; // Toplam İnşaat Alanı (m²)
  sgkLaborRatePercent?: number; // Asgari İşçilik Oranı (%) - ör: %9
  unitAreaEstimatedCost?: number; // m² Yaklaşık Birim Maliyeti (₺)
  contractorSgkFileNumber?: string; // Şantiye SGK Dosya No
}
...
// ==========================================
// 🏢 DEMİRBAŞLAR VE AMORTİSMAN YÖNETİMİ TİPLERİ (VUK & TMS UYUMLU)
// ==========================================

export type FixedAssetCategory =
  | "253_tesis_makine" // 253 Tesis, Makine ve Cihazlar
  | "254_tasitlar" // 254 Taşıtlar (Binek, Ticari, İş Makinesi)
  | "255_demirbaslar" // 255 Demirbaşlar (Büro Makinesi, Mobilya, Bilgisayar)
  | "252_binalar" // 252 Binalar ve Tesisler
  | "258_yapilmakta_olan" // 258 Yapılmakta Olan Yatırımlar
  | "260_haklar_lisans" // 260 Haklar ve Lisanslar
  | "264_ozel_maliyetler"; // 264 Özel Maliyetler

export type DepreciationMethod =
  | "normal" // Normal (Doğrusal / Eşit Paylı) Amortisman
  | "declining_balance" // Azalan Bakiyeler (Hızlandırılmış) Amortisman
  | "none"; // Amortismana Tabi Değil (Arsa vb.)

export type AssetConditionStatus =
  | "active" // Aktif Kullanımda
  | "in_custody" // Personele / Şantiyeye Zimmetli
  | "in_storage" // Depoda / Boşta
  | "maintenance" // Serviste / Bakımda
  | "scrapped" // Hurdaya Ayrıldı
  | "sold"; // Satıldı / Devredildi

export interface FixedAsset {
  id: string;
  code: string; // DMR-2024-001
  name: string; // Apple MacBook Pro 16", Renault Kangoo, vb.
  category: FixedAssetCategory;
  categoryLabel?: string;
  subCategory?: string; // Bilgisayar, Binek Araç, Mobilya, CNC Makine vb.
  brand?: string;
  model?: string;
  serialNumber?: string;
  barcode?: string;
  qrCode?: string;
  plateNumber?: string; // Taşıtlar için plaka

  // Satın Alma & Maliyet
  acquisitionDate: string; // YYYY-MM-DD
  activatedDate: string; // Aktife Giriş Tarihi (Amortisman başlangıcı)
  invoiceNo?: string;
  invoiceDate?: string;
  vendorId?: string;
  vendorName?: string;
  purchaseCost: number; // KDV Hariç Alış Bedeli
  vatRate: number; // 0, 1, 10, 20
  vatAmount: number;
  totalCost: number; // Genel Toplam Bedel
  currency: string; // TRY, USD, EUR
  additionalExpenses?: number; // Nakliye, Montaj vb.

  // Muhasebe Hesap Kodları (Tek Düzen)
  assetAccountCode: string; // örn: 255.01.001
  depreciationAccountCode: string; // örn: 257.01.001
  expenseAccountCode: string; // örn: 770.05.001

  // Amortisman Parametreleri (VUK Uyumlu)
  depreciationMethod: DepreciationMethod;
  usefulLifeYears: number; // Faydalı Ömür (Yıl) örn: 5
  depreciationRate: number; // Yıllık Amortisman Oranı (%) örn: 20
  isPartialYear: boolean; // Kıst Amortisman (Binek Otomobiller için ay esası)
  salvageValue?: number; // Hurda / Kalıntı Değeri
  accumulatedDepreciation: number; // Şu ana kadar ayrılmış toplam birikmiş amortisman
  netBookValue: number; // Net Defter Değeri (purchaseCost - accumulatedDepreciation)

  // Enflasyon Düzeltmesi / Yeniden Değerleme (VUK Geçici 32 & 298/Ç)
  revaluedCost?: number;
  revaluedAccumulatedDepr?: number;
  lastRevaluationDate?: string;

  // Lokasyon & Zimmet
  location?: string; // Merkez Ofis, Ankara Şube, İzmir Fabrika, Şantiye vb.
  branchId?: string;
  branchName?: string;
  warehouseId?: string;
  warehouseName?: string;
  department?: string; // Muhasebe, Bilgi İşlem, Şantiye, Satış
  custodyEmployeeId?: string;
  custodyEmployeeName?: string;
  custodyDate?: string;

  // Durum
  status: AssetConditionStatus;
...
export interface AssetDepreciationYearRecord {
  year: number;
  openingBookValue: number; // Dönem Başı Değeri
  depreciationRate: number; // Uygulanan Oran %
  annualDepreciation: number; // Bu Yıl Ayrılan Amortisman
  q1Amount: number; // 1. Geçici Vergi Payı
  q2Amount: number; // 2. Geçici Vergi Payı
  q3Amount: number; // 3. Geçici Vergi Payı
  q4Amount: number; // 4. Geçici Vergi / Yıllık Kapanış Payı
  accumulatedDepreciation: number; // Yıl Sonu Birikmiş Amortisman
  closingNetBookValue: number; // Net Defter Değeri (Kalan)
  isPartialApplied?: boolean; // Kıst amortisman uygulandı mı
  isFinalYearCatchup?: boolean; // Kıst kalanının eklendiği son yıl mı
  journalEntryPreview?: {
    debitAccount: string; // 770 / 760 / 730
    debitTitle: string;
    creditAccount: string; // 257 Birikmiş Amortisman
    creditTitle: string;
    amount: number;
  };
}
...
export interface AssetMaintenanceRecord {
  ...
  assetName: string;
  assetCode: string;
  maintenanceType: "periodic" | "repair" | "inspection" | "calibration" | "insurance";
  title: string;
  date: string;
  cost: number;
  isCapitalized: boolean; // Demirbaş maliyetine mi eklendi yoksa doğrudan gider mi?
...
export interface AssetDisposalRecord {
  ...
  assetName: string;
  assetCode: string;
  disposalType: "sale" | "scrap" | "loss" | "donation";
  date: string;
  originalCost: number;
  accumulatedDepreciation: number;
  netBookValue: number;
  salePrice: number; // Satış yapıldıysa satış bedeli
  gainOrLoss: number; // salePrice - netBookValue (Pozitif: 679 Kar, Negatif: 689 Zarar)
  invoiceNo?: string;
  customerName?: string;
  buyerTaxId?: string;
  notes?: string;
...
export const ALL_APP_MODULES: AppModuleDefinition[] = [
  ...
  { key: "construction_costing", label: "İnşaat Maliyetlendirme", description: "Şantiye, metraj, keşif, kaba/ince maliyet ve hakediş yönetimi", category: "Ticari" },
...
export interface FoodIngredientItem {
  ...
  amount?: number; // Alias for portionGrams or recipe quantity
  unit: "g" | "kg" | "ml" | "lt" | "adet" | "paket" | string;
...
export interface FoodRecipe {
  ...
  instructions?: string; // Pişirme talimatı (kısa alias)
  storageTemp?: string; // Servis sıcaklığı (+65°C sıcak / +4°C soğuk)
  servingTemp?: string; // Servis sıcaklığı alias
  isApproved: boolean; // Gıda Mühendisi / Aşçıbaşı Onayı
  status?: string; // Onay durumu ("approved" vb.)
  approvedBy?: string;
  equipment?: string[];
...
export interface FoodProductionOrder {
  ...
  stockDeductionDate?: string;
  stockDeductionSummary?: string;
  notes?: string;
  createdAt: string;
}
...
// ==========================================
// 🏗️ İNŞAAT MALİYETLENDİRME MODÜLÜ TİPLERİ
// ==========================================

export type ConstructionCategory =
  | "rough" // Kaba İnşaat (Hafriyat, Beton, Demir, Kalıp, Duvar, Çatı)
  | "fine" // İnce İnşaat (Sıva, Şap, Seramik, Boya, Doğrama, Kapı, Mutfak)
  | "mechanical_electrical" // Mekanik & Elektrik Tesisat (Sıhhi tesisat, Isıtma, Kablolama, Yangın, Asansör)
  | "permit_project" // Ruhsat, Proje & Harçlar (Mimari/Statik proje, Yapı denetim, Belediye harçları)
  | "site_overhead"; // Şantiye Genel Giderleri & İSG (Şantiye şefi, Vinç, Konteyner, Elektrik/Su)

export interface ConstructionCostItem {
  id: string;
  code?: string; // Örn: 01.01, KBA-01, POZ-101 vb.
  category: ConstructionCategory;
  name: string; // Örn: C30/37 Hazır Beton, Nervürlü İnşaat Demiri (Q12-Q24)
  unit: string; // m³, ton, m², mt, adet, adam/gün, paket
  quantity: number;
  unitPrice: number;
  totalPrice: number;
  contractorName?: string; // Taşeron / Tedarikçi Firma
  status: "estimated" | "contracted" | "in_progress" | "completed";
  paidAmount?: number;
  notes?: string;
  materialGroup?: string; // Örn: Hazır Beton, İnşaat Demiri, Kalıp & İskele, Yalıtım, Seramik & Kaplama, Tesisat
  targetUnitPrice?: number; // Optimize / Hedef Birim Fiyat (TL)
  savingsPotential?: number; // Tasarruf Potansiyeli (TL)
  savingsStrategy?: string; // Tasarruf Aksiyonu / Stratejisi (Örn: Doğrudan üretici alımı, alternatif malzeme)
  savingsFeasibility?: "high" | "medium" | "low"; // Uygulanabilirlik Seviyesi
  savingsStatus?: "pending" | "approved" | "rejected"; // Değerlendirme Durumu
}

export interface ConstructionProgressPayment {
  id: string;
  projectId: string;
  subcontractorName: string; // Taşeron Unvanı
  workScope: string; // Örn: Kalıp & Demir İşçiliği Hakedişi
  paymentNo: number; // Hakediş No (1, 2, 3...)
  period: string; // Örn: Eylül 2026
  grossAmount: number; // Brüt Hakediş Tutarı
  withholdingTaxDeduction: number; // Stopaj Kesintisi (%5)
  guaranteeDeduction: number; // Teminat Kesintisi (%6)
...
export interface CommonAreaItem {
  id: string;
  name: string; // Örn: Kapalı Otopark & Manevra Alanı
  category: "parking" | "shelter" | "stairs_circulation" | "elevator" | "mechanical" | "entrance_common" | "other";
  regulationNotice: string; // Yasal Yönetmelik Maddesi
  m2: number;
...
export interface ApartmentRoomItem {
  id: string;
  name: string; // Örn: Oturma Odası (Salon), Mutfak, Banyo vb.
  minM2: number; // İmar Yönetmeliği Madde 29 asgari yasal net alanı
  minWidthMeter?: number; // Asgari dar kenar ölçüsü (m)
  calculatedM2: number; // Projedeki m²
  isCompliant: boolean; // Yasal asgari sınırın üstünde mi
}

export interface ApartmentUnitConfig {
  id: string;
  type: string; // "1+1", "2+1", "3+1", "4+1", "5+1", "dublex" vb.
  title: string; // Örn: "2+1 Standart Aile Dairesi"
  count: number; // Adet
  targetNetM2: number; // Daire net m² (süpürülebilir alan)
  grossMultiplier: number; // Örn: 1.25 (brüt alan çarpanı)
  totalNetM2: number; // count * targetNetM2
  rooms: ApartmentRoomItem[]; // Piyes listesi
  estimatedSalePricePerUnit?: number; // Daire başı tahmini satış bedeli (TL)
}

export interface ConstructionProject {
  id: string;
  projectCode: string; // Örn: INS-2026-001
  projectName: string; // Örn: Kartal Panora Konutları - 48 Daire
  location: string; // Örn: İstanbul / Kartal / Karlıktepe
  projectType: "residential" | "commercial" | "villa" | "industrial" | "mixed"; // Konut, Ticari, Villa, Sanayi, Karma
  status: "planning" | "rough_phase" | "finishing_phase" | "completed";
  
  // Metraj & Alan Bilgileri
  landAreaM2: number; // Arsa Alanı (m²)
  totalConstructionAreaM2: number; // Toplam İnşaat Alanı (m²)
  sellableAreaM2: number; // Satılabilir Net Alan (m²)
  unitCount: number; // Bağımsız Bölüm Sayısı
  floorsCount: number; // Kat Sayısı (Bodrum + Zemin + Katlar)

  startDate: string;
  estimatedEndDate: string;

  // Maliyet & Bütçe
  estimatedCostPerM2: number; // Hedef m² Birim Maliyet (TL/m²)
  totalEstimatedCost: number; // Toplam Tahmini Maliyet (TL)
  realizedCost: number; // Gerçekleşen Maliyet (TL)
  estimatedSaleRevenue?: number; // Tahmini Satış Hasılatı (TL)
