...
const lineXml = (item: InvoiceItem, index: number, currency: string) => {
  ...
  const disc = numberValue(item.discountAmount, 0);
  const withoutVat = numberValue(item.totalWithoutVat, Math.max(0, gross - disc));
...
export function buildUblInvoiceXml(options: UblInvoiceBuildOptions): string {
  ...
  const rawProfile = options.profile || invoice.profile;
  const profile = isArchive && !rawProfile
    ? "EARSIVFATURA"
    : resolveScenarioProfileCode(String(rawProfile || (isArchive ? "EARSIVFATURA" : "TEMELFATURA")));
  ...
  const totalDiscount = numberValue(invoice.totalDiscount, items.reduce((sum, item) => sum + numberValue(item.discountAmount, 0), 0));
