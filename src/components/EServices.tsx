...
export const EServices: React.FC<EServicesProps> = ({
  ...
  const handleSaveNewTebligat = () => {
    const safeDocTitle = String(newTebligat.documentTitle || "").trim();
    const safeSenderUnit = String(newTebligat.senderUnit || "").trim();
    if (!safeDocTitle || !safeSenderUnit) return;
    const currentList = getTebligatlar();
    const created: ETebligatItem = {
      id: `eteb_${Date.now()}`,
      authority: newTebligat.authority || "GIB",
      senderUnit: safeSenderUnit,
      documentTitle: safeDocTitle,
      documentType: newTebligat.documentType || "İhbarname",
      barcodeNumber: String(newTebligat.barcodeNumber || "").trim() || `${newTebligat.authority || "GIB"}-${Date.now().toString().slice(-7)}`,
      envelopeId: String(newTebligat.envelopeId || "").trim() || `e-ZRF-${Date.now().toString().slice(-6)}`,
  ...
  return (
    <div className="p-3 sm:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      {/* Header Banner */}
      <div
        className="relative overflow-hidden rounded-2xl p-6 shadow-2xs border"
        style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}
      >
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1 flex flex-col items-center text-center">
            <div className="flex items-center justify-center gap-2 mb-1.5">
              <span className="bg-[#eaedff] text-[#0f6bae] border border-[#dae2fd] text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
            ...
            <h1 className="text-xl sm:text-2xl font-editorial font-medium flex items-center justify-center gap-2" style={{ color: theme.pageText }}>
              <ShieldCheck className="w-6 h-6 text-[#0f6bae]" />
              E-İşlemler ve Resmi Kurum Şifreleri
            </h1>
            <p className="text-xs mt-1 max-w-2xl leading-relaxed text-center" style={{ color: theme.pageTextMuted }}>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2">
        <button
          ...
          style={activeMainTab === "embedded" ? { backgroundColor: theme.primaryColor } : undefined}
        >
        ...
        <button
          ...
          style={activeMainTab === "credentials" ? { backgroundColor: theme.primaryColor } : undefined}
        >
        ...
        <button
          ...
          style={activeMainTab === "eTebligat" ? { backgroundColor: theme.primaryColor } : undefined}
        >
      ...
      <form onSubmit={handleSubmit} className={`space-y-6 ${activeMainTab === "embedded" ? "hidden" : "block"}`}>
        ...
        <div className="flex items-center justify-between bg-[#eaedff]/60 p-4 rounded-2xl border border-[#dae2fd] shadow-xs">
          {isSaved ? (
            ...
            <span className="text-xs text-[#0f6bae] font-medium">
          ...
          <button
            type="submit"
            className="text-white font-bold px-6 py-2.5 rounded-xl shadow-xs cursor-pointer transition-all flex items-center gap-2 hover:brightness-110"
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-sm font-black text-slate-800 flex items-center gap-2">
                    <span className="flex items-center justify-center w-7 h-7 rounded-xl bg-[#eaedff] text-[#0f6bae] font-bold text-xs shadow-2xs">
                      4
                    </span>
                    <span className="flex items-center gap-2">
                      <Landmark className="w-4 h-4 text-[#0f6bae]" />
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
                <div className="flex flex-wrap items-center gap-1.5">
                  <button
                    ...
                    style={tebligatFilter === "all" ? { backgroundColor: theme.primaryColor } : undefined}
                  >
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                            <button
                              ...
                              className="bg-[#eaedff] hover:bg-[#dae2fd] text-[#0f6bae] font-bold px-3 py-1.5 rounded-xl border border-[#dae2fd] transition-all flex items-center gap-1 cursor-pointer shadow-2xs text-xs"
                            >
