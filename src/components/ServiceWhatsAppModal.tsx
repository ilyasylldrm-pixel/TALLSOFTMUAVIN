...
export const ServiceWhatsAppModal: React.FC<ServiceWhatsAppModalProps> = ({
  ...
  const handleSendDirect = async () => {
    if (!cleanPhone || !message.trim()) {
      triggerFormErrorNotification("Lütfen geçerli bir telefon numarası ve mesaj girin.", "WhatsApp Form Uyarısı", "Telefon / Mesaj");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        triggerFormErrorNotification(`WhatsApp doğrudan gönderim hatası: ${res.error || "Bilinmeyen hata"}. Dilerseniz 'WhatsApp'ta Aç ve Gönder' seçeneğini kullanabilirsiniz.`, "WhatsApp İletim Hatası");
      }
    } catch (e: any) {
      triggerFormErrorNotification("Hata: " + e.message, "WhatsApp Hatası");
    } finally {
