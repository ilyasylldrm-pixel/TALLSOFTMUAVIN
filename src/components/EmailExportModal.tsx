...
export const EmailExportModal: React.FC<EmailExportModalProps> = ({
  ...
  const handleDownloadPDFOnly = async () => {
    if (!getPdfBlob) {
      triggerFormErrorNotification("PDF oluşturma işlevi bulunamadı.", "PDF Hatası");
      return;
    }
  ...
  const handleSendMailto = async () => {
    if (!recipientEmail.trim() || !isValidEmail(recipientEmail)) {
      triggerFormErrorNotification("Lütfen geçerli bir alıcı e-posta adresi giriniz.", "E-posta Form Hatası", "Alıcı E-posta");
      return;
    }
  ...
  const handleNativeShare = async () => {
    ...
    try {
      ...
      const res = await getPdfBlob();
      if (!res) {
        triggerFormErrorNotification("PDF oluşturulamadı.", "PDF Hatası");
        setIsProcessing(false);
        return;
      }
  ...
  return (
    ...
    >
      <div
        className="rounded-3xl shadow-sm border max-w-2xl mx-auto overflow-hidden transition-colors"
        style={{
      ...
      >
        ...
        <div className="p-4 sm:p-6 overflow-y-auto custom-scrollbar space-y-4 text-xs">
          ...
          {contacts.length > 0 && (
            <div>
              <label className="block font-bold mb-1" style={{ color: theme.pageText }}>
                Kayıtlı Cari Hesap Seçin (Otomatik Doldur)
              </label>
              <select
                onChange={(e) => handleSelectContact(e.target.value)}
                defaultValue=""
                className="w-full border rounded-xl px-3 py-2 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                style={{
        ...
        <div
          className="p-3.5 sm:p-4 px-4 sm:px-6 border-t shrink-0 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 z-20"
          style={{
            backgroundColor: theme.pageBg,
            borderColor: theme.cardBorder,
          }}
        >
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-xl border text-xs font-bold transition-colors cursor-pointer text-center"
            style={{
              backgroundColor: theme.cardBg,
              borderColor: theme.cardBorder,
              color: theme.pageText,
          ...
          <div className="flex items-center gap-2 flex-col sm:flex-row w-full sm:w-auto">
            {typeof navigator !== "undefined" && typeof navigator.share === "function" && (
              <button
                ...
                disabled={isProcessing || !recipientEmail || !isValidEmail(recipientEmail)}
                className="w-full sm:w-auto bg-purple-50 dark:bg-purple-950/40 hover:bg-purple-100 text-purple-900 dark:text-purple-200 border border-purple-300 dark:border-purple-800 px-3.5 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer transition-colors disabled:opacity-50 shadow-2xs"
                title="Sistem Paylaşım Menüsü / Doğrudan Dosya Eki ile Paylaş"
              >
                <Share2 className="w-4 h-4 text-purple-700 dark:text-purple-400" />
            ...
            <button
              ...
              onClick={handleSendMailto}
              disabled={isProcessing || !recipientEmail || !isValidEmail(recipientEmail)}
              className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors shadow-md disabled:opacity-50"
            >
              <Send className="w-4 h-4" />
