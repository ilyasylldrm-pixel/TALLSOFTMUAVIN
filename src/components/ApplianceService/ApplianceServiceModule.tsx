...
export const ApplianceServiceModule: React.FC<ApplianceServiceModuleProps> = ({
  ...
  const handleSaveRecord = (e: React.FormEvent) => {
    ...
    if (!formData.brand?.trim() || !formData.contactName?.trim() || !formData.customerProblemDescription?.trim()) {
      triggerFormErrorNotification("Lütfen Marka/Model, Müşteri Adı ve Arıza Tanımını eksiksiz doldurunuz.", "Beyaz Eşya Servis Formu Eksik Bilgi");
      return;
    }
  ...
  const handleRunAiPrompt = async (action: "field_checklist" | "quote_approval_message" | "completion_report") => {
    ...
    try {
      ...
      triggerFormErrorNotification("AI servisiyle iletişim kurulurken bir sorun oluştu.", "Yapay Zeka Servis Hatası");
    } finally {
      setAiLoading(false);
    }
  };
