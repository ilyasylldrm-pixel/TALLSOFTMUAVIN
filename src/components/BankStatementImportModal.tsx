...
export const BankStatementImportModal: React.FC<BankStatementImportModalProps> = ({
  ...
  const handleExecuteImport = () => {
    if (!selectedAccountId) {
      triggerFormErrorNotification("Lütfen ekstrenin yükleneceği banka hesabını seçiniz.", "Banka Hesabı Seçilmedi", "Banka Hesabı");
      return;
    }
    if (selectedRows.length === 0) {
      triggerFormErrorNotification("İçe aktarmak için en az bir hareket seçmelisiniz.", "Hareket Seçilmedi");
      return;
    }
    ...
    triggerFormErrorNotification(`${selectedRows.length} adet banka ekstresi hareketi başarıyla ${accName} hesabına işlendi.`, "Banka Ekstresi İçe Aktarım");
    onClose();
  };
