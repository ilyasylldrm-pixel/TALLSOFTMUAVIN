...
interface FoodRecipesViewProps {
  ...
  onNavigateToProductMrp?: (recipeId?: string) => void;
  onTriggerWorkOrder?: (recipes: FoodRecipe[], defaultPortion?: number) => void;
}
...
export const FoodRecipesView: React.FC<FoodRecipesViewProps> = ({
  ...
  const selectAllFiltered = (list: FoodRecipe[]) => {
    setSelectedRecipeIds(list.map((r) => r.id));
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2.5">
          {onTriggerWorkOrder && (
            ...
            >
              ...
              <span>
                {selectedRecipeIds.length > 0
                  ? `⚡ Seçili (${selectedRecipeIds.length}) Yemekle İş Emri Aç`
                  : "⚡ Yemek Listesinden İş Emri Tetikle"}
              </span>
            </button>
          )}
          {onNavigateToAnalysis && (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <button
                    ...
                    title={selectedRecipeIds.includes(recipe.id) ? "Seçimi Kaldır" : "İş Emri Listesi İçin Seç"}
                  >
                    {selectedRecipeIds.includes(recipe.id) ? (
                ...
                <div className="flex items-center gap-1">
                  {onNavigateToProductMrp && (
                    <button
                      type="button"
                      onClick={() => onNavigateToProductMrp(recipe.id)}
                      className="p-1.5 text-amber-700 hover:bg-amber-50 rounded-lg transition-colors cursor-pointer"
                      title="Maliyet Analizi ve MRP Kontrolü (Stok Açığı & Tedarik)"
                    >
                ...
                <div className="flex items-center gap-1.5 ml-auto">
                  {onNavigateToProductMrp && (
                    <button
                      type="button"
                      onClick={() => onNavigateToProductMrp(recipe.id)}
                      className="px-2.5 py-1 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 font-semibold text-2xs flex items-center gap-1 transition-all cursor-pointer"
                      title="Maliyet Analizi ve MRP Kontrolünü Aç"
                    >
                      <Scale className="w-3 h-3 text-amber-600" />
                      <span>MRP Kontrolü</span>
                    </button>
                  )}
                  ...
                  {onTriggerWorkOrder && (
                    <button
                      type="button"
                      onClick={() => onTriggerWorkOrder([recipe])}
                      className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200/80 font-bold text-2xs flex items-center gap-1.5 transition-all cursor-pointer active:scale-95"
                      title="Bu yemek için mutfak iş emri aç ve hammaddeleri stoktan düş"
                    >
                      <Sparkles className="w-3 h-3 text-amber-600" />
                      <span>İş Emri Aç & Stok Düş</span>
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
              <div className="flex items-center gap-2">
                ...
                {onNavigateToProductMrp && (
                  <button
                    type="button"
                    onClick={() => {
                      const recId = selectedRecipeForCalc.id;
                      setSelectedRecipeForCalc(null);
                      onNavigateToProductMrp(recId);
                    }}
                    className="px-3.5 py-2 text-xs font-semibold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <Scale className="w-4 h-4 text-amber-600" />
                    <span>Eksik Stok & MRP Kontrolünü Aç</span>
                  </button>
                )}
              </div>
              ...
              {onTriggerWorkOrder && (
                <button
                  type="button"
                  onClick={() => {
                    const rec = selectedRecipeForCalc;
                    setSelectedRecipeForCalc(null);
                    onTriggerWorkOrder([rec], calcPortionCount);
                  }}
                  className="px-4 py-2 text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                >
                  <Sparkles className="w-4 h-4 text-amber-200" />
                  <span>{calcPortionCount} Porsiyonla İş Emri Başlat & Stok Düş</span>
                </button>
              )}
            </div>
          </div>
      ...
      {/* Çoklu Yemek Seçildiğinde Yüzen İş Emri Çubuğu */}
      {selectedRecipeIds.length > 0 && onTriggerWorkOrder && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900/95 backdrop-blur-md text-white px-5 py-3.5 rounded-2xl shadow-2xl border border-slate-700/80 flex items-center gap-4 animate-in slide-in-from-bottom-4 duration-200 max-w-xl w-[92%]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-xs font-bold text-white block">
                {selectedRecipeIds.length} Yemek Seçildi
              </span>
              <span className="text-2xs text-slate-400 block truncate max-w-xs">
                {recipes
          ...
          <div className="flex items-center gap-2 ml-auto shrink-0">
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium text-slate-400 hover:text-white rounded-lg transition-colors cursor-pointer"
            >
              Temizle
            </button>
            <button
              type="button"
              onClick={() => {
                const list = recipes.filter((r) => selectedRecipeIds.includes(r.id));
                onTriggerWorkOrder(list);
              }}
              className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-extrabold px-4 py-2 rounded-xl text-xs flex items-center gap-1.5 shadow-lg shadow-amber-500/20 transition-all cursor-pointer active:scale-95"
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>Mutfak İş Emri Başlat & Stok Düş</span>
            </button>
          </div>
        </div>
      )}
