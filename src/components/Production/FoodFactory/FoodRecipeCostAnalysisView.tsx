  ...
  const allDistinctIngredients = useMemo(() => {
    ...
    recipes.forEach((rec) => {
      rec.ingredients.forEach((ing) => {
        const normalizedName = String(ing.name || "").trim();
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xl font-editorial font-medium text-slate-900 flex items-center gap-2">
                  ...
                  {selectedRecipe.ingredients.map((ing) => {
                    ...
                    return (
                      <tr key={ing.id} className="hover:bg-slate-50">
                        ...
                        <td className="py-2 px-3 text-right font-bold text-amber-900 font-mono tabular-nums">
                          {netRequiredKg >= 1 ? `${netRequiredKg.toFixed(1)} kg` : `${(netRequiredKg * 1000).toFixed(0)} gr`}
                        </td>
                        <td className="py-2 px-3 text-right text-orange-600 font-medium font-mono tabular-nums">
                          {ing.wastagePercent > 0 ? `%${ing.wastagePercent}` : "-"}
                        </td>
                        <td className="py-2 px-3 text-right font-black text-slate-900 font-mono tabular-nums">
                          {grossRequiredKg >= 1
                        ...
                        <td className="py-2 px-3 text-right font-bold text-emerald-800 font-mono tabular-nums">
