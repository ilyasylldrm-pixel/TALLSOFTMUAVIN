...
export const WhatsAppCenter: React.FC<WhatsAppCenterProps> = ({ settings }) => {
  ...
  const handleConnect = async (force = false) => {
    ...
    try {
      ...
      triggerFormErrorNotification("Bağlantı başlatılamadı: " + err.message, "WhatsApp Bağlantı Hatası");
    } finally {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      triggerFormErrorNotification("Oturum kapatılamadı: " + err.message, "WhatsApp Oturum Hatası");
    } finally {
  ...
  const handleSendTest = async (e: React.FormEvent) => {
    ...
    if (!testPhone.trim() || !testMessage.trim()) {
      triggerFormErrorNotification("Lütfen telefon numarası ve mesaj metni girin.", "WhatsApp Form Hatası", "Telefon / Mesaj");
      return;
    }
  ...
  const handleSaveTemplates = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerFormErrorNotification("Şablonlar kaydedilemedi: " + err.message, "Şablon Kayıt Hatası");
    } finally {
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-emerald-950 via-teal-950 to-slate-900 rounded-3xl p-6 text-white shadow-xl border border-emerald-800/40">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 flex-1 text-center">
            ...
            <h1 className="text-2xl sm:text-3xl font-editorial font-medium text-white flex items-center justify-center gap-3">
              <MessageSquare className="w-8 h-8 text-emerald-400" />
              WhatsApp İletişim & Entegrasyon Merkezi
            </h1>
            <p className="text-xs sm:text-sm text-emerald-200/80 max-w-2xl leading-relaxed mx-auto text-center">
