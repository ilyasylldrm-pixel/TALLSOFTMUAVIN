...
export const WhatsAppTemplateLibrary: React.FC<WhatsAppTemplateLibraryProps> = ({
  ...
  const setFormError = (msg: string | null) => {
    ...
    if (msg) {
      triggerFormErrorNotification(msg, 'Şablon Formu Hatası');
    }
  };
  ...
  const handleSaveTemplate = (e: React.FormEvent) => {
    e.preventDefault();
    const safeTitle = String(formTitle || "").trim();
    const safeText = String(formText || "").trim();
    const safeShortcut = String(formShortcut || "").trim();
    const safeDescription = String(formDescription || "").trim();
      ...
      const updated = templates.map(t => {
        if (t.id === editingTemplate.id) {
          return {
            ...
            text: formText,
            shortcut: safeShortcut ? (safeShortcut.startsWith('/') ? safeShortcut : `/${safeShortcut}`) : undefined,
        ...
        >
          <div className="bg-white w-full max-w-4xl mx-auto rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col p-6">
            ...
            <form onSubmit={handleSaveTemplate} className="p-6 space-y-6 overflow-y-auto max-h-[80vh]">
              {formError && (
                <div className="haze-form-error-toast p-3.5 rounded-xl border border-[#fecdd3] text-[#7f1d1d] text-xs font-bold flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <Info className="w-4 h-4 shrink-0 text-[#b91c1c]" />
                    <span>{formError}</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => setFormError(null)}
                    className="text-[#991b1b] hover:text-[#7f1d1d] font-bold px-1 rounded hover:bg-[#b91c1c]/10 cursor-pointer"
                  >
                    ✕
                  </button>
                </div>
