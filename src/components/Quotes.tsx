...
export const Quotes: React.FC<QuotesProps> = ({
  ...
  const [formType, setFormType] = useState<"proforma" | "quote">("quote");
  ...
  const [notes, setNotes] = useState("Teklif geçerlilik süresi 30 gündür.");

  const [items, setItems] = useState([
    {
      id: "qi_1",
      productId: products[0]?.id || "",
      description: products[0]?.name || "Yazılım Entegrasyonu Teklifi / Proforma Faturası",
      quantity: 1,
  ...
  const openNewFormModal = (type: "quote" | "proforma") => {
    setFormType(type);
    setNotes(
      type === "quote"
        ? "Teklif geçerlilik süresi 30 gündür. Bakiye sipariş onayında %50 peşin, %50 teslimatta tahsil edilir."
        : "Proforma fatura geçerlilik süresi 30 gündür."
    );
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-purple-50 via-fuchsia-50/40 to-slate-50/80 rounded-2xl p-5 border border-purple-200/60 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0 relative z-10">
          <button
            onClick={() => openNewFormModal("quote")}
            className="bg-purple-700/15 hover:bg-purple-700/25 text-purple-950 border border-purple-400/50 backdrop-blur-md font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 shadow-2xs cursor-pointer transition-all shrink-0"
          >
            <Plus className="w-4 h-4 text-purple-800 font-bold" />
            <span>Yeni Teklif Formu Hazırla</span>
          </button>
      ...
      <div className="bg-white rounded-2xl border border-purple-200/60 shadow-2xs overflow-hidden space-y-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative w-full max-w-sm">
            ...
            <input
              type="text"
              placeholder="Teklif / Proforma no veya Müşteri ara..."
              value={search}
          ...
          <ExportButtons
            getExportData={() => ({
              filename: `Teklifler_ve_Proformalar_${new Date().toISOString().split("T")[0]}`,
              title: "TEKLİFLER VE PROFORMA FATURALAR LİSTESİ",
              ...
              rows: filteredQuotes.map((q) => {
                ...
                return [
                  q.number,
                  q.type === "proforma" ? "Proforma Fatura" : "Satış Teklifi",
                  q.contactName,
                  q.issueDate,
                  q.validUntil,
              ...
              {filteredQuotes.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-8 text-slate-400 bg-white rounded-xl border border-purple-100/80">
                    Kayıtlı teklif veya proforma fatura bulunmuyor.
                  </td>
                </tr>
              ) : (
          ...
          <div className="bg-white border border-purple-200/80 text-slate-900 rounded-2xl max-w-4xl w-full p-6 shadow-2xl space-y-5 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-purple-100 pb-3">
              <h3 className="text-base font-extrabold text-slate-900">
                {formType === "quote" ? "Yeni Teklif Formu Oluştur" : "Yeni Proforma Fatura Oluştur"}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-purple-50/50 p-4 rounded-xl border border-purple-100">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    {formType === "quote" ? "Teklif Tarihi *" : "Proforma Tarihi *"}
                  </label>
                  <input
                    type="date"
                    required
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold uppercase text-purple-950 tracking-wider flex items-center gap-1.5">
                    ...
                    <span>Teklif Kalemleri & Stok / Hizmet Seçimi</span>
                  </h4>
                  <button
                    type="button"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  {formType === "quote" ? "Teklif Notu / Şartlar" : "Proforma Notu / Şartlar"}
                </label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Teklif şartları, ödeme planı veya teslimat notları..."
              ...
              <div className="pt-2 flex justify-end gap-2 border-t border-purple-100">
                ...
                >
                  {formType === "quote" ? "Teklif Formunu Kaydet" : "Proforma Faturayı Kaydet"}
                </button>
              </div>
            </form>
          </div>
      ...
      {printingQuote && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white border border-slate-200 text-slate-900 rounded-2xl max-w-4xl w-full p-8 shadow-2xl space-y-6 my-8 print:p-0 print:shadow-none print:m-0 print:w-full print:max-w-none print:border-none print:bg-white print:text-black">
            ...
            <div className="flex items-center justify-between border-b border-purple-100 pb-4 print:hidden">
              <div className="flex items-center gap-2">
                <span className={`text-xs font-black px-2.5 py-1 rounded-md uppercase tracking-wide border ${
                  printingQuote.quoteNumber.startsWith("TEK")
                    ? "bg-fuchsia-50 text-fuchsia-800 border-fuchsia-200"
                    : "bg-purple-50 text-purple-800 border-purple-200"
                }`}>
                  {printingQuote.quoteNumber.startsWith("TEK") ? "Teklif Formu" : "Proforma Fatura"}
                </span>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="bg-purple-700 hover:bg-purple-800 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 shadow-xs cursor-pointer transition-colors"
                >
            ...
            <div id="printable-quote" className="bg-white text-slate-900 p-8 border border-purple-100 rounded-xl space-y-6 print:border-none print:p-0">
              {/* Header Banner */}
              <div className="flex items-start justify-between border-b-2 border-purple-950 pb-6">
                ...
                <div className="text-right space-y-2">
                  <div className="inline-block bg-purple-950 text-white px-4 py-2 rounded-lg font-black text-sm uppercase tracking-wider">
                    {printingQuote.quoteNumber.startsWith("TEK") ? "TEKLİF FORMU" : "PROFORMA FATURA"}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-8 pt-8 border-t border-purple-100 text-center text-xs">
                <div className="space-y-12">
                  <div className="font-bold text-slate-900">Teklifi Sunan / Firma Yetkilisi</div>
                  <div className="border-b border-dashed border-slate-300 mx-8"></div>
                  <div className="text-[10px] text-slate-400">İmza / Kaşe</div>
                </div>
                <div className="space-y-12">
                  <div className="font-bold text-slate-900">Teklifi Onaylayan / Müşteri Yetkilisi</div>
                  <div className="border-b border-dashed border-slate-300 mx-8"></div>
