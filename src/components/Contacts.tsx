...
import { ExportData, formatCurrency, formatDate, sanitizeOklchForHtml2Canvas, exportElementToPDF, generateAccountStatementAutoTablePDF, exportElementToPDFWithPrintStyling, LedgerSummaryData, exportToExcel, exportToPDF } from "../utils/exportUtils";
...
interface ContactsProps {
  ...
  onBulkAddContacts?: (contacts: Contact[], updateExisting?: boolean) => void;
...
const CONTACT_TABLE_COLUMNS: ColumnDef[] = [
  { id: "contact", label: "Cari Kodu & Unvanı", alwaysVisible: true },
  { id: "type", label: "Cari Tipi", defaultVisible: true },
  { id: "contactInfo", label: "İletişim & Adres", defaultVisible: true },
  { id: "risk", label: "Risk Limiti", defaultVisible: true },
  { id: "balance", label: "Güncel Bakiye", defaultVisible: true },
...
export const Contacts: React.FC<ContactsProps> = ({
  ...
  // Master-Detail Expanded Rows State (set of expanded contact IDs)
  ...
  const handleImportContacts = (importedContacts: Contact[], updateExisting: boolean) => {
    if (onBulkAddContacts) {
      onBulkAddContacts(importedContacts, updateExisting);
    } else {
      importedContacts.forEach((c) => onAddContact(c));
    }
  };

  const nav = useDetailNavigation<Contact>({ moduleKey: "contacts" });
  ...
  const handleSaveAction = (e: React.FormEvent) => {
    ...
    if (payMethod === "kasa" || payMethod === "banka" || payMethod === "kredi_karti") {
      ...
      if (!virmanTargetContactId) {
        triggerFormErrorNotification("Lütfen virman yapılacak karşı cari hesabı seçin.", "Cari Virman Hatası", "Hedef Cari");
        return;
      }
  ...
  const currentComputedAccountCode = useMemo(() => {
    const customCode = String(formData.accountCode || "").trim();
    if (formData.isCustomAccountCode && customCode) {
      return customCode;
    }
    ...
    const taxNum = String(formData.taxNumber || "").trim() || "0000000000";
  ...
  const handleOpenAddModal = (contact?: Contact) => {
    if (contact) {
      ...
      const hasCustomCode = Boolean(contact.accountCode && String(contact.accountCode).trim());
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const finalName = String(formData.name || "").trim() || String(formData.companyTitle || "").trim();
    if (!finalName) return;
    ...
    const contactPayload: Partial<Contact> = {
      name: finalName,
      companyTitle: String(formData.companyTitle || "").trim() || finalName,
      companyName: String(formData.companyTitle || "").trim() || finalName,
  ...
  const activeColFilterCount = useMemo(() => {
    return (Object.values(colFilters) as string[]).filter((v) => Boolean(v && v.trim() !== "")).length;
  }, [colFilters]);
  ...
  const {
    ...
    columnVisibility: contactColVisibility,
    isVisible: isContactColVisible,
    toggleColumn: toggleContactCol,
    setAllColumns: setAllContactCols,
    resetToDefaults: resetContactCols,
    hiddenCount: hiddenContactColsCount,
  } = useColumnVisibility("contacts", CONTACT_TABLE_COLUMNS);

  const visibleContactColCount = useMemo(() => {
    return 1 + contactColumns.filter((col) => isContactColVisible(col.id)).length;
  }, [contactColumns, isContactColVisible]);
    ...
    return contacts.filter((c) => {
      ...
      if (colFilters.contact) {
        ...
        const matchesCol =
          ...
          accountCodeStr.includes(q) ||
          (c.companyTitle && c.companyTitle.toLowerCase().includes(q)) ||
          (c.taxNumber && c.taxNumber.toLowerCase().includes(q));
        if (!matchesCol) return false;
      }

      if (colFilters.type) {
        const q = colFilters.type.toLowerCase().trim();
        const typeStr =
      ...
      if (colFilters.contactInfo) {
        const q = colFilters.contactInfo.toLowerCase().trim();
        const infoStr = `${c.phone || ""} ${c.mobile || ""} ${c.email || ""} ${c.city || ""} ${c.contactPerson || ""}`.toLowerCase();
        if (!infoStr.includes(q)) return false;
      }

      if (colFilters.risk) {
        const q = colFilters.risk.toLowerCase().trim();
        const limitStr = String(c.creditLimit || "");
        const riskRatio = c.balance > 0 ? Math.min(100, Math.round((c.balance / 150000) * 100)) : 0;
        const riskLevel = riskRatio > 80 ? "yüksek risk" : riskRatio > 45 ? "orta risk" : "düşük risk güvenli";
        if (!riskLevel.includes(q) && !limitStr.includes(q)) return false;
      }

      if (colFilters.balance) {
        const q = colFilters.balance.toLowerCase().trim();
        const balNum = c.balance.toLocaleString("tr-TR", { minimumFractionDigits: 2 });
        const balRaw = String(c.balance);
        const balStatus = c.balance > 0 ? "alacak alacaklı" : c.balance < 0 ? "borç borçlu" : "sıfır";
        if (!balNum.includes(q) && !balRaw.includes(q) && !balStatus.includes(q)) return false;
      }
  ...
  }, [contacts, activeSearchQuery, filterType, onlyDebtors, statusFilter, colFilters]);
  ...
  const displayedContactIds = useMemo(() => {
    return displayedContacts.map((c) => c.id);
  }, [displayedContacts]);
  ...
  const handleSendWhatsApp = async () => {
    ...
    if (!cleanPhone || cleanPhone.length < 10) {
      triggerFormErrorNotification("Lütfen geçerli bir telefon numarası girin.", "WhatsApp Form Uyarısı", "Telefon");
      return;
    }
    ...
    try {
      ...
      if (!pdfResult) {
        triggerFormErrorNotification("PDF ekstre oluşturulurken bir hata oluştu. Lütfen tekrar deneyin.", "PDF Oluşturma Hatası");
      ...
      if (whatsappMode === "direct") {
        ...
        if (sendRes.success) {
          ...
          setCloudStatusText("");
          setShareType(null);
          triggerFormErrorNotification(`PDF Cari Ekstre belgesi ("${pdfResult.fileName}") ve bilgilendirme mesajı ${selectedLedgerContact.name} (+${cleanPhone}) WhatsApp hattına doğrudan başarıyla iletildi!`, "WhatsApp İletimi");
          return;
        } else {
          setIsGeneratingPDF(false);
          setCloudStatusText("");
          triggerFormErrorNotification(`WhatsApp doğrudan gönderim hatası: ${sendRes.error || "Bilinmeyen hata"}. Dilerseniz 'WhatsApp Web' sekmesini seçerek manuel gönderebilirsiniz.`, "WhatsApp İletim Hatası");
          return;
        }
      }
      ...
      if (whatsappMode === "native" || (navigator.canShare && navigator.canShare({ files: [pdfFile] }))) {
        if (navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
          ...
          triggerFormErrorNotification("Doğrudan dosya eki ile sistem paylaşım menüsü desteklenmiyor. PDF belgesi cihazınıza indirildi ve WhatsApp Web açılıyor.", "WhatsApp Paylaşımı");
        }
      }
      ...
      setTimeout(() => {
        ...
        setCloudStatusText("");
        setShareType(null);
        triggerFormErrorNotification(`PDF Cari Ekstre belgesi ("${pdfResult.fileName}") indirildi ve WhatsApp Web açıldı!`, "WhatsApp Paylaşımı");
      }, 500);
    } catch (err) {
      console.error("WhatsApp paylaşım hatası:", err);
      triggerFormErrorNotification("WhatsApp ile paylaşım sırasında bir hata oluştu.", "Paylaşım Hatası");
      setIsGeneratingPDF(false);
      setCloudStatusText("");
    }
  };
  ...
  const handleSendEmail = async () => {
    if (!shareEmail.trim() || !shareEmail.includes("@")) {
      triggerFormErrorNotification("Lütfen geçerli bir e-posta adresi girin.", "E-posta Form Uyarısı", "E-Posta Adresi");
      return;
    }
    ...
    try {
      ...
      triggerFormErrorNotification("E-posta paylaşımı sırasında bir hata oluştu.", "E-posta Gönderim Hatası");
    }
  };

  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false);

  const getContactsExportData = (targetContacts: Contact[] = filteredContacts): ExportData => {
    const headers = [
      "Cari Hesap No",
      "Cari Adı / Unvanı",
      "Firma Unvanı",
    ...
    return {
      ...
      subtitle: `Toplam ${targetContacts.length} Adet Cari Hesap Kaydı`,
      headers,
      rows,
    };
  };
  ...
  const handleBulkExportExcel = () => {
    const selected = filteredContacts.filter((c) => selection.isSelected(c.id));
    exportToExcel(getContactsExportData(selected));
  };

  const handleBulkExportPdf = () => {
    const selected = filteredContacts.filter((c) => selection.isSelected(c.id));
    exportToPDF(getContactsExportData(selected));
  };
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          body { font-family: 'Inter', system-ui, -apple-system, sans-serif; padding: 24px; color: #131b2e; margin: 0; background: #ffffff; }
          .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #005289; padding-bottom: 16px; margin-bottom: 20px; page-break-inside: avoid; break-inside: avoid; }
          .company-name { font-size: 20px; font-weight: 800; text-transform: uppercase; color: #005289; letter-spacing: -0.5px; }
          .title-block { text-align: right; }
          .title { font-size: 20px; font-weight: 800; color: #131b2e; letter-spacing: 0.5px; text-transform: uppercase; }
          .info-grid { display: grid; grid-template-columns: 1.2fr 1.2fr 1fr; gap: 16px; background: #faf8ff; padding: 16px; border-radius: 6px; border: 1.5px solid #dae2fd; margin-bottom: 20px; font-size: 12px; page-break-inside: avoid; break-inside: avoid; }
          .info-box { display: flex; flex-direction: column; justify-content: center; }
          .balance-box { background: #ffffff; border: 1.5px solid #c0c7d2; border-radius: 6px; padding: 12px; text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center; }
          table { width: 100%; border-collapse: collapse; margin-top: 10px; border: 1.5px solid #c0c7d2; border-radius: 6px; overflow: hidden; }
          th { background: #005289; color: #ffffff; text-align: center; padding: 10px 12px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; border-right: 1px solid #0f6bae; }
          ...
          .stat-box { background: #faf8ff; border: 1.5px solid #dae2fd; border-radius: 6px; padding: 12px; text-align: center; }
          .stat-box.highlight { background: #eaedff; border-color: #0f6bae; }
          .stat-label { font-size: 10px; font-weight: 700; color: #005289; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
  ...
  if (isModalOpen) {
    return (() => {
        ...
        return (
          <DetailPageLayout
            title={editingContact ? `${editingContact.name} - Cari Kartı Düzenle` : "Yeni Cari Kart Oluştur"}
            subtitle="10 Adımlı Resmi Cari Bilgi Sıralaması (Otomatik 120/320 Cari Kodu & VKN/TCKN Entegrasyonu)"
            breadcrumbs={[
              { label: "Cari Hesaplar", onClick: handleBackToList },
              { label: editingContact ? editingContact.name : "Yeni Cari Kart", active: true },
            ]}
            onBack={handleBackToList}
            statusBadge={
              <span className="px-3 py-1 rounded-full font-mono font-bold bg-purple-100 text-purple-800 border border-purple-200 text-xs">
                {currentComputedAccountCode}
              </span>
            }
            headerIcon={<Building2 className="w-5 h-5 text-purple-700" />}
            actions={
              ...
              <form id="contact-form" onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {formData.isCustomAccountCode ? (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1.5 text-[10px] font-sans font-medium px-2 py-0.5 rounded border border-purple-500/20 bg-purple-500/5 text-purple-600">
                              ...
                              <span>•</span>
                              <span><b>{String(formData.taxNumber || "").trim() || "0000000000"}</b></span>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2 rounded-xl text-xs font-bold bg-purple-700 hover:bg-purple-800 text-white shadow-sm cursor-pointer transition-colors flex items-center gap-1.5"
                    >
                      <Check className="w-4 h-4" />
                      <span>{editingContact ? "Güncelle" : "Cari Hesabı Kaydet"}</span>
                    </button>
  ...
  if (selectedLedgerContact) {
    return (
        <DetailPageLayout
          title={`${selectedLedgerContact.name} - Cari Hesap Ekstresi & Muavin Dökümü`}
          subtitle={`Hesap No: ${getContactAccountCode(selectedLedgerContact)} • VKN/TCKN: ${selectedLedgerContact.taxNumber || "-"} • Net Bakiye: ₺${Math.abs(ledgerSummary?.netBalance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}`}
          breadcrumbs={[
            { label: "Cari Hesaplar", onClick: handleBackToList },
            { label: selectedLedgerContact.name, active: true },
          ]}
          onBack={handleBackToList}
          statusBadge={
            <span
          ...
          headerIcon={<Building2 className="w-5 h-5 text-purple-600" />}
          actions={
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs px-3.5 py-2 rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
        ...
        >
          ...
          >
            ...
            <div className="p-2 sm:p-6 bg-slate-100 dark:bg-slate-900/50 rounded-2xl overflow-y-auto custom-scrollbar flex-1 flex justify-center">
              <div
                id="printable-ledger"
                className="bg-white text-slate-900 p-4 sm:p-8 rounded-2xl shadow-xl border border-slate-200 w-full max-w-4xl mx-auto space-y-4 sm:space-y-5 font-sans text-xs sm:text-sm"
                style={{ backgroundColor: "#ffffff", color: "#0f172a" }}
              >
                {/* 3.1 Corporate Header */}
                <div
                  className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 pb-4 border-b-2 border-slate-800"
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h2
                          className="text-sm sm:text-base md:text-lg font-black tracking-tight uppercase"
                          style={{ color: "#1e1b4b" }}
                        >
                    ...
                    <div className="text-[10px] sm:text-[11px] pl-11 space-y-0.5" style={{ color: "#475569" }}>
                  ...
                  <div
                    className="text-left sm:text-right p-2.5 sm:p-3 rounded-xl border space-y-1 shrink-0 w-full sm:w-auto"
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            color: (ledgerSummary?.netBalance || 0) > 0 ? "#047857" : (ledgerSummary?.netBalance || 0) < 0 ? "#be123c" : "#334155",
                          }}
                        >
                          ₺{Math.abs(ledgerSummary?.netBalance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                      <div className="space-y-1 pt-0.5 text-[11px]">
                        <div className="flex justify-between items-center px-1">
                          <span style={{ color: "#64748b" }} className="font-medium">Toplam Borçlandırılan Tutar:</span>
                          <span className="font-mono font-bold" style={{ color: "#1e293b" }}>₺{(ledgerSummary?.totalDebit || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between items-center px-1">
                          <span style={{ color: "#64748b" }} className="font-medium">Toplam Alacaklandırılan Tutar:</span>
                          <span className="font-mono font-bold" style={{ color: "#1e293b" }}>₺{(ledgerSummary?.totalCredit || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between items-center px-1">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  >
                    ...
                    <span className="text-sm sm:text-base font-black font-mono block" style={{ color: "#172554" }}>
                      ₺{(ledgerSummary?.totalDebit || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                    </span>
                  ...
                  >
                    ...
                    <span className="text-sm sm:text-base font-black font-mono block" style={{ color: "#1e1b4b" }}>
                      ₺{(ledgerSummary?.totalCredit || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                    </span>
                        ...
                        {filteredLedgerEntries.length === 0 ? (
                          ...
                          filteredLedgerEntries.map((m, idx) => (
                            ...
                            >
                              ...
                              <td className="py-2 px-3 font-mono tabular-nums font-bold whitespace-nowrap" style={{ color: "#1e293b" }}>
                                {m.documentNo || "-"}
                              </td>
                              <td className="py-2 px-3 max-w-[220px] truncate" title={m.description} style={{ color: "#334155" }}>
                                {m.description || "-"}
                              </td>
                              <td className="py-2 px-3 text-right font-mono tabular-nums font-tabular-num-md font-bold whitespace-nowrap" style={{ color: "#0f172a" }}>
                              ...
                              <td className="py-2 px-3 text-right font-mono tabular-nums font-tabular-num-md font-black whitespace-nowrap" style={{ backgroundColor: "#f1f5f9" }}>
                                <span
                                  style={{
                      ...
                      {filteredLedgerEntries.length > 0 && (
                        <tfoot>
                          <tr
                            ...
                            style={{ backgroundColor: "#eaedff", color: "#131b2e", borderTopColor: "#005289" }}
                          >
                            ...
                            <td className="py-2.5 px-3 text-right font-mono text-xs">
                              ₺{(filteredLedgerEntries.reduce((acc, curr) => acc + (curr.debit || 0), 0) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                            </td>
                            <td className="py-2.5 px-3 text-right font-mono text-xs">
                              ₺{(filteredLedgerEntries.reduce((acc, curr) => acc + (curr.credit || 0), 0) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                            </td>
                            <td className="py-2.5 px-3 text-right font-mono text-xs" style={{ backgroundColor: "#dae2fd", color: "#005289" }}>
                              ₺{Math.abs(ledgerSummary?.netBalance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                            </td>
                          </tr>
                        </tfoot>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 pt-4 border-t" style={{ borderTopColor: "#e2e8f0" }}>
                  <div className="text-center space-y-4 sm:space-y-8">
                    ...
                    <div className="h-8 sm:h-10 border-b border-dashed w-36 mx-auto" style={{ borderBottomColor: "#cbd5e1" }}></div>
                  </div>
            ...
            <div
              className="p-3 sm:p-4 px-4 sm:px-6 border-t rounded-2xl shrink-0 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 z-20 no-print"
              style={{
            ...
            >
              <span className="text-xs font-bold" style={{ color: theme.pageText }}>
                ...
                <strong className={(ledgerSummary?.netBalance || 0) > 0 ? "text-emerald-600" : (ledgerSummary?.netBalance || 0) < 0 ? "text-rose-600" : "text-slate-500"}>
                  ₺{Math.abs(ledgerSummary?.netBalance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                </strong>
              </span>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-end">
                <button
                  ...
                  onClick={handleExportLedgerPDFDirect}
                  disabled={isPdfGenerating}
                  className="bg-purple-700 hover:bg-purple-800 text-white text-xs font-bold px-3.5 sm:px-4 py-2 rounded-xl cursor-pointer transition-colors flex items-center gap-1.5 shadow-xs disabled:opacity-50"
                ...
                <button
                  type="button"
                  onClick={() => handleOpenShareModal("whatsapp")}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 sm:px-3.5 py-2 rounded-xl cursor-pointer transition-colors flex items-center gap-1.5"
                >
                ...
                <button
                  type="button"
                  onClick={() => handleOpenShareModal("email")}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 sm:px-3.5 py-2 rounded-xl cursor-pointer transition-colors flex items-center gap-1.5 shadow-xs"
                >
                ...
                <button
                  type="button"
                  onClick={handleBackToList}
                  className="px-3.5 sm:px-4 py-2 rounded-xl border text-xs font-bold transition-colors cursor-pointer"
                  style={{
  ...
  if (shareType === "whatsapp" && selectedLedgerContact) {
    const defaultMsg = `Sayın *${selectedLedgerContact.name}* (${getContactAccountCode(selectedLedgerContact)}),\n\n*${companySettings?.companyName || "Firma"}* firmamıza ait Cari Hesap Ekstreniz tanzim edilmiştir.\n\n📊 *Güncel Net Bakiye:* ${formatCurrency(Math.abs(selectedLedgerContact.balance), companySettings?.currency || "TRY")} (${selectedLedgerContact.balance > 0 ? "Alacaklıyız" : selectedLedgerContact.balance < 0 ? "Borçluyuz" : "Sıfır Bakiye"})\n\n📄 *Ekstre PDF Dosyası:* "${selectedLedgerContact.name.replace(/\s+/g, "_")}_Cari_Ekstre.pdf" ekte yer almaktadır.`;

    return (
      <UniversalWhatsAppModal
        isOpen={true}
        onClose={() => setShareType(null)}
        title="WhatsApp ile Cari Ekstre Paylaş"
        subtitle={`${selectedLedgerContact.name} (${getContactAccountCode(selectedLedgerContact)})`}
        breadcrumbs={[
          { label: "Cari Hesaplar", onClick: () => setShareType(null) },
          { label: selectedLedgerContact.name, onClick: () => setShareType(null) },
          { label: "WhatsApp Paylaşımı", active: true },
        ]}
        documentTypeLabel="Cari Hesap Ekstresi & Muavin Dökümü"
        recipientName={selectedLedgerContact.name}
        recipientPhone={sharePhone || selectedLedgerContact.phone || selectedLedgerContact.mobile || ""}
        defaultMessage={defaultMsg}
        documentFileName={`${selectedLedgerContact.name.replace(/\s+/g, "_")}_Cari_Ekstre_${new Date().getFullYear()}.pdf`}
        companySettings={companySettings}
        quickTemplates={[
          {
            id: "standard_ekstre",
            label: "📋 Standart Cari Ekstre Bildirimi",
            templateText: defaultMsg,
          },
          {
            id: "reminder",
            label: "⏰ Bakiye Hatırlatması & Ödeme Bildirimi",
            templateText: `Sayın *${selectedLedgerContact.name}*,\n\n*${companySettings?.companyName || "Firma"}* nezdindeki cari hesabınızda ${formatCurrency(Math.abs(selectedLedgerContact.balance), companySettings?.currency || "TRY")} tutarında vadesi gelmiş/yaklaşan bakiyeniz bulunmaktadır.\n\nDetaylı hesap ekstreniz ekteki PDF dosyasındadır. Ödemenizi gerçekleştirmeniz veya mutabakat teyidi vermeniz rica olunur.\n\nSaygılarımızla,\n*${companySettings?.companyTitle || companySettings?.companyName || "Finans Yönetimi"}*`,
          },
          {
            id: "reconciliation",
            label: "🤝 Aylık Dönem Mutabakatı",
            templateText: `Sayın Yetkili (${selectedLedgerContact.name}),\n\nŞirketimiz ile aranızdaki ${new Date().toLocaleDateString("tr-TR", { month: "long", year: "numeric" })} dönemi cari hesap mutabakat mektubu ve hesap dökümü ekte yer almaktadır.\n\nKayıtlarımızda bakiyeniz *${formatCurrency(Math.abs(selectedLedgerContact.balance), companySettings?.currency || "TRY")}* olarak görünmektedir. Mutabık olup olmadığınızı teyit etmenizi rica ederiz.\n\nSaygılarımızla,\n*${companySettings?.companyTitle || companySettings?.companyName || "Finans Yönetimi"}*`,
          },
        ]}
        onGeneratePdf={async () => {
          const res = await generateLedgerPDF(selectedLedgerContact);
          return res ? { blob: res.blob, fileName: res.fileName } : null;
        }}
        onSuccess={() => setShareType(null)}
      />
    );
  }
  ...
  if (actionModalType && selectedActionContact) {
    return (
        <DetailPageLayout
          ...
          <div className="bg-white rounded-3xl max-w-4xl w-full p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6 mx-auto">
            ...
            <div className="bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div className="sm:text-right">
                ...
                >
                  ₺{Math.abs(selectedActionContact?.balance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                </div>
                      ...
                      {accounts
                        ...
                        .map((acc) => (
                          <option key={acc.id} value={acc.id}>
                            ...
                            {(acc.balance ?? 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                          </option>
                        ))}
                      {accounts.filter((a) =>
                        payMethod === "kasa"
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6 sm:space-y-7">
      {/* Header Bar With Editorial Background */}
      <ModuleEntranceHeader
        badge="Müşteri & Tedarikçi"
        badgeIcon={<Users className="w-2.5 h-2.5 text-[#0f6bae]" />}
        title="Cari Kartlar"
        description="Müşteri ve tedarikçi hesapları, bakiye ve risk takibi."
        actions={
          <>
            <ExportButtons
            ...
            <button
              ...
              title="Excel veya CSV dosyasından carileri toplu içe aktar"
            >
            ...
            <button
              ...
              className="text-xs font-semibold text-white px-3.5 py-2.5 rounded-xl transition-all shadow-xs flex items-center gap-1.5 cursor-pointer hover:opacity-90 active:scale-95 shrink-0"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          ...
          className="rounded-2xl p-5 border shadow-2xs hover:shadow-md transition-all cursor-pointer group relative overflow-hidden haze-kpi-card-bg"
          style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold mt-1.5 tracking-tight text-emerald-600 font-mono">
                ₺{(totalReceivable || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
              </h3>
            </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold mt-1.5 tracking-tight text-rose-600 font-mono">
                ₺{(totalPayable || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
              </h3>
            </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold mt-1.5 tracking-tight text-amber-600 font-mono">
                ₺{(overdueReceivableTotal || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
              </h3>
            </div>
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          <ColumnManagementDropdown
            columns={contactColumns}
            columnVisibility={contactColVisibility}
            onToggleColumn={toggleContactCol}
            onSetAllColumns={setAllContactCols}
            onResetToDefaults={resetContactCols}
            hiddenCount={hiddenContactColsCount}
          />
          <ExportButtons
            getExportData={getContactsExportData}
      ...
      <BulkActionBar
        ...
        onSelectAll={() => selection.selectAll(filteredContacts.map((c) => c.id))}
        onDelete={handleBulkDelete}
        deleteLabel="Seçilen Carileri Sil"
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[700px]">
            <thead>
              ...
              >
                <th className="w-10 py-3 px-3 text-center">
                  <TableCheckbox
                    checked={displayedContactIds.length > 0 && selection.isAllSelected(displayedContactIds)}
                    indeterminate={selection.isIndeterminate(displayedContactIds)}
                    onChange={() => selection.toggleSelectAll(displayedContactIds)}
                    title="Tümünü Seç / Seçimi Kaldır"
                  />
                </th>
                {isContactColVisible("contact") && <th className="py-3 px-4">Cari</th>}
                {isContactColVisible("type") && <th className="py-3 px-3">Tip</th>}
                {isContactColVisible("contactInfo") && <th className="py-3 px-3">İletişim</th>}
                {isContactColVisible("risk") && <th className="py-3 px-3">Risk Limiti</th>}
                {isContactColVisible("balance") && <th className="py-3 px-4 text-right">Bakiye</th>}
                {isContactColVisible("actions") && <th className="py-3 px-4 text-center">İşlemler</th>}
              </tr>
              ...
              {showColFilters && (
                ...
                >
                  ...
                  {isContactColVisible("contact") && (
                    <th className="py-2 px-3 font-normal">
                      <TableColumnFilterInput
                        ...
                        placeholder="Cari / Kod / VKN ara..."
                      />
                    </th>
                  )}
                  {isContactColVisible("type") && (
                    <th className="py-2 px-2 font-normal w-28">
                      <TableColumnFilterInput
                        type="select"
                        value={colFilters.type}
                        onChange={(val) => setColFilters((prev) => ({ ...prev, type: val }))}
                        options={[
                          { label: "Tümü", value: "" },
                  ...
                  {isContactColVisible("contactInfo") && (
                    <th className="py-2 px-2 font-normal">
                      <TableColumnFilterInput
                        value={colFilters.contactInfo}
                        onChange={(val) => setColFilters((prev) => ({ ...prev, contactInfo: val }))}
                        placeholder="Tel / E-posta / İl..."
                      />
                    </th>
                  )}
                  {isContactColVisible("risk") && (
                    <th className="py-2 px-2 font-normal w-32">
                      <TableColumnFilterInput
                        value={colFilters.risk}
                        onChange={(val) => setColFilters((prev) => ({ ...prev, risk: val }))}
                        placeholder="Risk / Limit..."
                      />
                    </th>
                  )}
                  {isContactColVisible("balance") && (
                    <th className="py-2 px-3 font-normal w-32">
                      <TableColumnFilterInput
                        value={colFilters.balance}
                        onChange={(val) => setColFilters((prev) => ({ ...prev, balance: val }))}
                        placeholder="Bakiye tutarı..."
                      />
                    </th>
                  )}
                  {isContactColVisible("actions") && (
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredContacts.length === 0 ? (
                <tr>
                  <td colSpan={visibleContactColCount} className="py-12 text-center text-slate-400 text-xs">
                ...
                displayedContacts.map((c) => {
                  ...
                  const isRowSelected = selection.isSelected(c.id);
                  ...
                  const contactInvoices = invoices.filter(
                    (inv) => inv.contactId === c.id || inv.contactName?.toLowerCase() === c.name.toLowerCase()
                  );
                  ...
                  return (
                    <React.Fragment key={c.id}>
                      ...
                      >
                        ...
                        {/* Cari (Chevron + Name + Account Code / VKN) */}
                        {isContactColVisible("contact") && (
                        ...
                        {isContactColVisible("risk") && (
                          <td className="py-3.5 px-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[11px] font-mono tabular-nums font-bold" style={{ color: riskColor }}>
                                %{riskRatio}
                              </span>
                            </div>
                          </td>
                        ...
                        {/* Bakiye */}
                        {isContactColVisible("balance") && (
                          <td className="py-3.5 px-4 text-right whitespace-nowrap font-mono tabular-nums font-tabular-num-md">
                            <div
                              className={`font-mono tabular-nums font-tabular-num-md font-bold text-sm ${
                                isReceivable
                                  ? "text-emerald-600"
                                  : isPayable
                                  ? "text-rose-600"
                            ...
                            >
                              ...
                              {Math.abs(c.balance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                            </div>
                            ...
                            <div className="p-4 sm:p-6 bg-gradient-to-br from-slate-50 via-[#eaedff]/40 to-white border-l-4 border-l-[#005289] space-y-4">
                              ...
                              <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-200">
                                <div className="flex flex-wrap items-center gap-2.5">
                                  ...
                                  <span className="text-xs text-slate-500 font-medium">
                                    Kod: <span className="font-mono font-bold text-slate-900">{getContactAccountCode(c)}</span>
                                  </span>
                                  {c.taxNumber && (
                                    <span className="text-xs text-slate-500 font-mono">
                                      VKN/TCKN: <strong className="text-slate-800">{c.taxNumber}</strong>
                                    </span>
                                  )}
                                  {c.taxOffice && (
                                    <span className="text-xs text-slate-500">
                                      Vergi Dairesi: <strong className="text-slate-800">{c.taxOffice}</strong>
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="flex items-center gap-2">
                                  <button
                                    type="button"
                                    onClick={() => setSelectedLedgerContact(c)}
                                    className="px-2.5 py-1.5 rounded-lg text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs"
                                  >
                                    <FileSpreadsheet className="w-3.5 h-3.5" />
                                    <span>Cari Ekstre Sayfası</span>
                                  </button>
                                  {onOpenNewInvoiceForContact && (
                                    <button
                                      type="button"
                                      onClick={() => onOpenNewInvoiceForContact(c)}
                                      className="px-2.5 py-1.5 rounded-lg text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors flex items-center gap-1.5 cursor-pointer shadow-2xs"
                                    >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1.5">
                                  ...
                                  <div className="text-xs text-slate-800 space-y-1 font-medium">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="font-mono text-slate-900 font-semibold">{c.phone || "Belirtilmemiş"}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                      <span className="text-slate-400">E-Posta:</span>
                                      <span className="text-slate-700">{c.email || "Belirtilmemiş"}</span>
                                    </div>
                                    {c.iban && (
                                      <div className="flex items-center gap-1.5 text-[11px]">
                                        <span className="text-slate-400">IBAN:</span>
                                        <span className="font-mono text-slate-700 font-bold">{c.iban}</span>
                                      </div>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1.5">
                                  ...
                                  <div className="text-xs text-slate-700 font-medium">
                                    <p className="font-semibold text-slate-900">
                                      {c.district ? `${c.district} / ` : ""}{c.city || "Şehir belirtilmemiş"}
                                    </p>
                                    <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                                      {c.address || "Açık adres kaydı bulunmuyor."}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs space-y-1.5">
                                  ...
                                  <div className="flex justify-between items-baseline font-mono tabular-nums">
                                    <span className="text-xs text-slate-500 font-sans">Güncel Bakiye:</span>
                                    <span className={`text-base font-bold ${isReceivable ? "text-emerald-600" : isPayable ? "text-rose-600" : "text-slate-500"}`}>
                                      {isReceivable ? "+" : isPayable ? "-" : ""}₺{Math.abs(c.balance || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                                    </span>
                                  </div>
                                  <div className="text-[11px] text-slate-500 pt-1 border-t border-slate-100 flex justify-between font-mono tabular-nums">
                                    ...
                                    <span className="font-bold text-slate-800">{contactInvoices.length} Adet</span>
                                  </div>
                                </div>
                              </div>

                              {/* Recent Invoices for this Contact */}
                              {contactInvoices.length > 0 && (
                                <div className="space-y-1.5">
                                  <span className="text-[11px] font-bold uppercase text-slate-500 tracking-wider">
                                    Son Faturalar ({contactInvoices.length})
                                  </span>
                                  <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-2xs">
                                    <table className="w-full text-left text-xs">
                                      <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase font-bold border-b border-slate-100">
                                        <tr>
                                          <th className="py-2 px-3">Fatura No</th>
                                      ...
                                      <tbody className="divide-y divide-slate-100">
                                        {contactInvoices.slice(0, 3).map((inv) => (
                                          <tr key={inv.id} className="hover:bg-slate-50/50">
                                            ...
                                            <td className="py-2 px-3 font-mono text-slate-500">{inv.issueDate}</td>
                                            <td className="py-2 px-3 text-right font-mono tabular-nums font-bold text-slate-900">
                                              ₺{((inv.grandTotal ?? (inv as any).totalAmount) || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                                            </td>
                                            <td className="py-2 px-3 text-center">
                                              ...
                                              >
                                                {inv.status === "paid" ? "Ödendi" : inv.status === "overdue" ? "Gecikmiş" : "Bekliyor"}
                                              </span>
                                            </td>
                                          </tr>
                                        ))}
