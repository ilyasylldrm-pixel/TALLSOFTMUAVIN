...
export const AutoServiceModule: React.FC<AutoServiceModuleProps> = ({
  ...
  const handleSaveRecord = () => {
    if (!formData.plateNumber || !formData.brand) {
      triggerFormErrorNotification("Lütfen araç plaka ve marka bilgilerini giriniz.", "Araç Servis Formu Eksik Bilgi", "Plaka / Marka");
      return;
    }
  ...
  if (isModalOpen) {
    return (
        <DetailPageLayout
          ...
          headerIcon={<Car className="w-5 h-5" style={{ color: theme.primaryColor }} />}
          actions={
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      ...
      {activeTab === "records" && (
        <div className="space-y-4 animate-fadeIn">
          ...
          {/* Records Table */}
          <div 
            className="bg-white/90 backdrop-blur-md rounded-2xl border shadow-2xs overflow-hidden"
            style={{ borderColor: theme.cardBorder }}
          >
                  ...
                  {filteredServices.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="py-12 text-center text-slate-400">
                        <Car className="w-10 h-10 mx-auto mb-2 text-slate-300 opacity-60" />
                          ...
                          <tr className="hover:bg-slate-50/60 transition-colors">
                            <td className="py-3.5 px-3 text-center">
                              <button
                                ...
                                className="p-1.5 rounded-lg hover:bg-slate-200/70 text-slate-400 hover:text-slate-700 transition-all cursor-pointer"
                              ...
                              <td colSpan={9} className="p-4 sm:p-5">
                                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs space-y-4">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs pb-3 border-b border-slate-100">
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Şasi No (VIN):</span>
                                      <span className="font-mono font-bold text-slate-800">{record.vinNumber || "Belirtilmemiş"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Motor & Yakıt:</span>
                                      <span className="font-semibold text-slate-800">{record.engineSize || "-"} • {record.fuelType || "Benzin"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Atanan Teknisyen:</span>
                                      <span className="font-bold text-slate-800">{record.technicianName || "Genel Servis Ekibi"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Tahmini Teslim:</span>
                                  ...
                                  {record.diagnosticNotes && (
                                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs">
                                      <span className="font-bold text-slate-700 block mb-1">Usta Teşhisi & Arıza Tespiti:</span>
                                      <p className="text-slate-600">{record.diagnosticNotes}</p>
                                    </div>
                                  )}
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between font-bold text-slate-800">
                                        <span className="flex items-center gap-1.5">
                                          ...
                                          Kullanılan Yedek Parçalar ({record.parts?.length || 0})
                                        </span>
                                          ...
                                          record.parts.map((p, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-100 text-xs">
                                              <div>
                                                ...
                                                {p.partCode && <span className="text-[10px] text-slate-400 ml-1.5 font-mono">({p.partCode})</span>}
                                              </div>
                                              <div className="text-right">
                                                <span className="text-slate-500 mr-2">{p.quantity} adet x ₺{p.unitPrice.toLocaleString("tr-TR")}</span>
                                                <span className="font-bold text-slate-800 font-mono">₺{p.totalPrice.toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                                              </div>
                                            </div>
                                          ))
                                        ) : (
                                          <p className="text-slate-400 text-xs italic p-2">Kayıtlı yedek parça bulunmuyor.</p>
                                        )}
                                      </div>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between font-bold text-slate-800">
                                        <span className="flex items-center gap-1.5">
                                          ...
                                          Ustalık & Servis İşçilikleri ({record.labor?.length || 0})
                                        </span>
                                        <span className="font-mono text-slate-900">
                                      ...
                                      <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
                                        {record.labor && record.labor.length > 0 ? (
                                          record.labor.map((l, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-100 text-xs">
                                              <span className="font-semibold text-slate-800">{l.description}</span>
                                              <span className="font-bold text-slate-800 font-mono">₺{l.price.toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                                            </div>
                                          ))
                                        ) : (
                                          <p className="text-slate-400 text-xs italic p-2">Kayıtlı işçilik bulunmuyor.</p>
                                        )}
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-100 text-xs">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="font-black text-base font-mono text-slate-900">₺{record.grandTotal.toLocaleString("tr-TR", { minimumFractionDigits: 2 })}</span>
                                      {record.isApprovedByCustomer ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleOpenInvoicing(record)}
                                        className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-semibold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer border border-emerald-200"
                                      >
                                        <Receipt className="w-3.5 h-3.5 text-emerald-700" />
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => handleOpenWhatsApp(record)}
                                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer"
                                      >
                                      ...
                                      <button
                                        ...
                                        onClick={() => handleOpenEditModal(record)}
                                        style={{ backgroundColor: theme.primaryColor }}
                                        className="px-3 py-1.5 text-white font-semibold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer shadow-2xs hover:opacity-90"
                                      >
                                        <Edit3 className="w-3.5 h-3.5" />
                                        <span>İş Emrini Düzenle</span>
                                      </button>
                                    </div>
                                  </div>
                                </div>
