...
export const FileManager: React.FC<FileManagerProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 8 * 1024 * 1024) {
        triggerFormErrorNotification("Seçilen dosya çok büyük! Lütfen 8 MB'tan küçük bir dosya yükleyin.", "Dosya Boyutu Hatası", "Dosya Seçimi");
        e.target.value = "";
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (selectedFile.size > 8 * 1024 * 1024) {
      triggerFormErrorNotification("Seçilen dosya çok büyük! Lütfen 8 MB'tan küçük bir dosya seçin.", "Dosya Boyutu Hatası", "Dosya Seçimi");
      return;
    }
    ...
    try {
      ...
      if (msg.includes("payload") || msg.includes("exceeds") || msg.includes("11534336")) {
        triggerFormErrorNotification("Dosya Yükleme Hatası: Seçilen dosya boyutu sunucu yükleme sınırını (10 MB) aşıyor. Lütfen daha küçük bir dosya seçin.", "Dosya Boyutu Aşımı");
      } else {
        triggerFormErrorNotification("Dosya yüklenirken bir hata oluştu: " + msg, "Dosya Yükleme Hatası");
      }
    } finally {
      setUploading(false);
    }
  ...
  const handleDelete = async (file: UserFileMetadata) => {
    ...
    try {
      ...
      triggerFormErrorNotification("Dosya silinemedi. Güvenlik yetkilerini kontrol edin.", "Dosya Silme Hatası");
    }
  };
    ...
    <div className="p-3 sm:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col items-center justify-center text-center gap-4">
        <div className="flex-1 text-center max-w-3xl mx-auto">
          <h1 className="text-2xl font-editorial font-medium tracking-tight text-center" style={{ color: theme.pageText }}>
            Bulut Dosya Deposu & Fiş/Fatura Ayrıştırıcı
          </h1>
          <p className="text-xs font-medium mt-1 text-center" style={{ color: theme.pageTextMuted }}>
        ...
        <div
          className="p-2.5 px-4 rounded-2xl border shadow-2xs flex items-center justify-center gap-3 shrink-0"
