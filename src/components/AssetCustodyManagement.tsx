...
export const AssetCustodyManagement: React.FC<AssetCustodyManagementProps> = ({
  ...
  const handleDirectExportPDF = async (asset: AssetCustody, returnDoc = false) => {
    try {
      ...
      triggerFormErrorNotification("PDF belgesi oluşturulurken bir hata meydana geldi.", "PDF Belgesi Hatası");
    } finally {
  ...
  const handleExportInventoryExcel = () => {
    if (!assets || assets.length === 0) {
      triggerFormErrorNotification("Dışa aktarılacak zimmet ve demirbaş kaydı bulunamadı.", "Dışa Aktarma Uyarısı");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-editorial font-medium text-slate-800 tracking-tight">
            ...
            <table className="w-full text-left text-xs min-w-[850px]">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {filteredAssets.map((asset) => {
                  ...
                  return (
                    <tr key={asset.id} className="hover:bg-blue-50/80 transition-colors cursor-pointer">
