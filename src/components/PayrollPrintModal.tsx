  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (scope === "single" && currentEmp) {
        const sanitizedName = String(currentEmp.fullName || (currentEmp as any).name || "Personel").trim().replace(/\s+/g, "_");
    ...
    >
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3.5 bg-slate-50 rounded-xl border border-slate-300 text-xs mb-4">
                  ...
                  <div>
                    ...
                    {singleMonthlyRecord.isRetired && (
                      <span className="inline-block mt-0.5 px-1.5 py-0.2 bg-blue-100 text-blue-800 text-[9px] font-extrabold rounded">
                        SGDP (Emekli)
                      </span>
                    )}
                  </div>
                      ...
                      <tbody>
                        <tr className="border-b border-slate-200">
                          <td className="p-2 text-slate-700">
                            {singleMonthlyRecord.isRetired ? "SGDP İşçi Primi (%7.5):" : "SGK İşçi Primi (%14):"}
                          </td>
                          <td className="p-2 text-right text-slate-900">{formatTRY(singleMonthlyRecord.sgkEmployeeShare)}</td>
                        </tr>
                        <tr className="border-b border-slate-200">
                          <td className="p-2 text-slate-700">
                            {singleMonthlyRecord.isRetired ? "İşsizlik Primi (Muaf %0):" : "İşsizlik Sigortası Primi (%1):"}
                          </td>
                  ...
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs mb-4 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-800">
                        {singleMonthlyRecord.isRetired
                          ? `İşveren SGDP Payı (%24.5): ${formatTRY(singleMonthlyRecord.sgkEmployerShare)}`
                          : `İşveren SGK Payı (%15.5): ${formatTRY(singleMonthlyRecord.sgkEmployerShare)}`}
                      </span>
                    </div>
                    <div>
                      <span className="font-bold text-slate-800">
                        {singleMonthlyRecord.isRetired
                          ? `İşveren İşsizlik (Muaf %0): ${formatTRY(singleMonthlyRecord.unemploymentEmployerShare)}`
                          : `İşveren İşsizlik (%2): ${formatTRY(singleMonthlyRecord.unemploymentEmployerShare)}`}
                      </span>
                    </div>
      ...
      {currentEmp && (
        <UniversalWhatsAppModal
          ...
          documentFileName={`Maas_Pusulasi_${String(currentEmp?.fullName || (currentEmp as any)?.name || "Personel").trim().replace(/\s+/g, "_")}_${monthStr}.pdf`}
          companySettings={companySettings}
          onGeneratePdf={async () => {
            const { exportElementToPDFWithPrintStyling } = await import("../utils/pdfService");
            const fileName = `Maas_Pusulasi_${String(currentEmp?.fullName || (currentEmp as any)?.name || "Personel").trim().replace(/\s+/g, "_")}_${monthStr}.pdf`;
