      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl p-6 shadow-md relative overflow-hidden">
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl lg:text-3xl font-editorial font-medium tracking-tight">
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <button
              ...
              className="text-white font-semibold px-4 py-2.5 rounded-xl text-xs sm:text-sm shadow-sm transition-all flex items-center gap-2 cursor-pointer active:scale-95 hover:opacity-90"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {sectors.map((sec) => {
          ...
          return (
            <div
              key={sec.id}
              style={isCurrentActive ? { borderColor: theme.primaryColor } : { borderColor: theme.cardBorder }}
              className={`rounded-2xl border p-5 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between space-y-4 ${
                isCurrentActive
                  ? "bg-slate-50/70 ring-2 ring-blue-400/20"
                  : "bg-white"
              }`}
            >
              <div>
                ...
                <div className="flex items-start justify-between gap-3 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div
                      style={isCurrentActive ? { backgroundColor: "#eaedff", color: theme.primaryColor, borderColor: "#dae2fd" } : undefined}
                  ...
                  {isCurrentActive && (
                    <span
                      style={{ backgroundColor: "#eaedff", color: theme.primaryColor, borderColor: "#dae2fd" }}
                      className="px-2.5 py-0.5 rounded-full text-2xs font-bold border flex items-center gap-1"
                    >
              ...
              <div className="pt-3 border-t border-slate-100">
                {isCurrentActive ? (
                  <button
                    ...
                    onClick={onNavigateToProduction}
                    style={{ backgroundColor: theme.primaryColor }}
                    className="w-full py-2 hover:opacity-90 text-white font-semibold rounded-xl text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-2xs"
                  >
            ...
            <form onSubmit={handleCreateSector} className="space-y-3.5 text-xs">
              ...
              <div className="flex justify-end gap-2 pt-3 border-t">
                ...
                <button
                  type="submit"
                  style={{ backgroundColor: theme.primaryColor }}
                  className="px-5 py-2 hover:opacity-90 text-white font-semibold rounded-xl cursor-pointer shadow-2xs"
                >
                  Sektörü Kaydet
                </button>
              </div>
