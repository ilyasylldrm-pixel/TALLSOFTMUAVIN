...
export const ServiceDeliveryModal: React.FC<ServiceDeliveryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-sm overflow-y-auto animate-fadeIn">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[94vh] flex flex-col overflow-hidden my-auto animate-scaleUp">
        {/* HEADER CONTROLS (Screen Only) */}
        <div className="px-6 py-4 bg-gradient-to-r from-slate-900 via-purple-950 to-slate-900 text-white flex items-center justify-between shrink-0 print:hidden">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base sm:text-lg font-black tracking-tight text-white">
                  Ürün & Servis Teslim Tutanağı
                </h3>
                <span className="px-2.5 py-0.5 rounded-full text-[11px] font-mono font-bold bg-purple-500/30 text-purple-200 border border-purple-400/30">
                  {serviceRecord.serviceNo}
                </span>
                {hasInvoice && (
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-500/30 text-emerald-200 border border-emerald-400/30 flex items-center gap-1">
                    <Receipt className="w-3 h-3" />
                    Faturalandı: {invoiceNo}
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-300 font-medium">
                {serviceRecord.contactName} • {deliveryDate} {deliveryTime}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold shadow transition-all cursor-pointer"
            >
