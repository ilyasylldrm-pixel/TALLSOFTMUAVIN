...
export const AiExpenseScannerModal: React.FC<AiExpenseScannerModalProps> = ({
  ...
  const matchedContact = contacts.find(
    (c) =>
      ...
      (extractedData.companyTitle && c.name && c.name.toLowerCase() === String(extractedData.companyTitle || "").trim().toLowerCase())
  );
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 15 * 1024 * 1024) {
        triggerFormErrorNotification("Dosya boyutu çok büyük (Max: 15 MB). Lütfen daha küçük bir belge yükleyin.", "Dosya Boyutu Sınırı", "Belge Dosyası");
        return;
      }
                    ...
                    >
                      {accounts.map((acc) => (
                        <option key={acc.id} value={acc.id}>
                          {acc.name} ({acc.type === "cash" ? "Kasa" : acc.type === "bank" ? "Banka" : "POS"} - ₺{(acc.balance ?? 0).toLocaleString("tr-TR")})
                        </option>
                      ))}
                    </select>
