...
interface WaybillsProps {
  ...
  onBulkDeleteWaybills?: (ids: string[]) => void;
}
...
export const Waybills: React.FC<WaybillsProps> = ({
  ...
  const handleSaveWaybill = (e: React.FormEvent) => {
    ...
    if (!selectedContactId) {
      triggerFormErrorNotification("Lütfen irsaliye için bir Cari Firma / Kişi seçiniz.", "İrsaliye Form Hatası", "Cari Seçimi");
      return;
    }
  ...
  const displayedWaybillIds = React.useMemo(() => {
    return displayedWaybills.map((w) => w.id);
  }, [displayedWaybills]);
  ...
  // Export Data Builder
  const getWaybillsExportData = (targetWaybills: Waybill[] = filteredWaybills): ExportData => {
    const headers = [
      "İrsaliye No",
      "İrsaliye Tipi",
      "İrsaliye Tarihi",
    ...
    const rows = targetWaybills.map((w) => [
      w.waybillNumber,
    ...
    return {
      ...
      subtitle: activeTab === "all" ? `Tüm İrsaliyeler (${targetWaybills.length} İrsaliye)` : activeTab === "dispatch" ? `Sevk İrsaliyeleri (${targetWaybills.length} İrsaliye)` : `Alış İrsaliyeleri (${targetWaybills.length} İrsaliye)`,
      headers,
      rows,
    };
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (onBulkDeleteWaybills) {
      onBulkDeleteWaybills(ids);
    } else {
      ids.forEach((id) => onDeleteWaybill(id));
    }
    selection.clearSelection();
  };

  const handleBulkExportExcel = () => {
    const selected = filteredWaybills.filter((w) => selection.isSelected(w.id));
    exportToExcel(getWaybillsExportData(selected));
  };

  const handleBulkExportPdf = () => {
    const selected = filteredWaybills.filter((w) => selection.isSelected(w.id));
    exportToPDF(getWaybillsExportData(selected));
  };
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      ...
      <ModuleEntranceHeader
        badge={
          forcedType === "receipt"
            ? "Alış & Tedarik İrsaliyeleri"
            : forcedType === "dispatch"
            ? "Satış & Sevk İrsaliyeleri"
            : "Lojistik & Taşıma"
        }
        badgeIcon={<Truck className="w-2.5 h-2.5 text-[#0f6bae]" />}
        title={pageTitle}
          ...
          <>
            {(!forcedType || forcedType === "dispatch") && (
              <button
                ...
                className="text-white font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 shadow-xs cursor-pointer transition-all hover:opacity-90 active:scale-95 shrink-0"
            ...
            {(!forcedType || forcedType === "receipt") && (
              <button
                ...
                className="text-white font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 shadow-xs cursor-pointer transition-all hover:opacity-90 active:scale-95 bg-amber-600 hover:bg-amber-700 shrink-0"
              >
                <Plus className="w-4 h-4" />
      ...
      <BulkActionBar
        ...
        onSelectAll={() => selection.selectAll(filteredWaybills.map((w) => w.id))}
        onDelete={handleBulkDelete}
        deleteLabel="Seçilen İrsaliyeleri Sil"
          ...
          <table className="w-full text-left text-xs border-separate border-spacing-y-2.5 min-w-[800px]">
            <thead>
              <tr className="text-purple-950 font-extrabold uppercase tracking-wider text-[11px]">
                <th className="pb-2 px-3 text-center w-10">
                  <TableCheckbox
                    checked={displayedWaybillIds.length > 0 && selection.isAllSelected(displayedWaybillIds)}
                    indeterminate={selection.isIndeterminate(displayedWaybillIds)}
                    onChange={() => selection.toggleSelectAll(displayedWaybillIds)}
            ...
            <tbody>
              {filteredWaybills.length === 0 ? (
                <tr>
                  <td colSpan={9} className="text-center py-8 text-slate-400 bg-white rounded-xl border border-purple-100/80">
                    Kriterlere uygun irsaliye kaydı bulunamadı.
                  </td>
                </tr>
              ) : (
                displayedWaybills.map((waybill) => {
                  const isRowSelected = selection.isSelected(waybill.id);
                  return (
                  <tr
                    key={waybill.id}
                    className={`${
                      isRowSelected
                        ...
                        : "bg-white hover:bg-gradient-to-r hover:from-purple-50/90 hover:via-fuchsia-50/60 hover:to-purple-50/90"
                    } hover:shadow-[0_1px_3px_rgba(15,23,42,0.04),0_1px_2px_rgba(15,23,42,0.02)] transition-all duration-200 group rounded-xl relative z-0 hover:z-10`}
                  >
                    <td
                      className={`py-3.5 px-3 rounded-l-xl border-y border-l text-center w-10 transition-all ${
                        isRowSelected
                          ? "border-indigo-400 bg-indigo-50/90 dark:bg-indigo-950/50"
                          : "border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30"
                      }`}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <TableCheckbox
                        checked={isRowSelected}
                        onChange={() => selection.toggleSelect(waybill.id)}
                        title="İrsaliyeyi Seç"
                      />
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-mono tabular-nums font-tabular-num-md">
                      <div className="font-extrabold text-slate-900 group-hover:text-purple-950 font-mono tabular-nums font-tabular-num-md text-sm">
                        ₺{waybill.grandTotal.toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                      </div>
                      <div className="text-[10px] text-slate-400 group-hover:text-purple-700/60 font-mono tabular-nums font-tabular-num-md">
