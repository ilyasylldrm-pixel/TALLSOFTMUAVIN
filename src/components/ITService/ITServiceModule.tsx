...
export const ITServiceModule: React.FC<ITServiceModuleProps> = ({
  ...
  const nav = useDetailNavigation<ItServiceRecord>({ moduleKey: "it-service" });
  ...
  const handleSaveRecord = () => {
    if (!formData.brand || !formData.model) {
      triggerFormErrorNotification("Lütfen cihaz marka ve model bilgilerini giriniz.", "Bilişim Servis Formu Eksik Bilgi", "Marka / Model");
      return;
    }
  ...
  if (isModalOpen) {
    return (
        <DetailPageLayout
          ...
          statusBadge={
            <span 
              className="px-3 py-1 text-xs font-bold rounded-xl border bg-slate-50 text-slate-700"
              style={{ borderColor: theme.cardBorder }}
            >
              {formData.brand && formData.model ? `${formData.brand} ${formData.model}` : "Yeni Cihaz"}
            </span>
          }
          headerIcon={<Laptop className="w-5 h-5" style={{ color: theme.primaryColor }} />}
          actions={
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-5 py-2 hover:opacity-90 text-white rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-2 active:scale-95"
              >
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-editorial font-medium tracking-tight" style={{ color: theme.pageText }}>
            Bilişim & BT Teknik Servis Yönetimi
          </h1>
          <p className="text-xs font-medium mt-1" style={{ color: theme.pageTextMuted }}>
            Cihaz kabulü, donanım & çip onarımı, veri güvenliği, yedek parça takibi ve BT servis raporları
          </p>
        </div>
        ...
        >
          <button
            ...
            style={activeTab === "records" ? { backgroundColor: theme.primaryColor, color: "#ffffff" } : {}}
          ...
          <button
            ...
            style={activeTab === "ai_assistants" ? { backgroundColor: theme.primaryColor, color: "#ffffff" } : {}}
            className={`font-bold text-xs py-2 px-3.5 rounded-xl flex items-center gap-2 cursor-pointer transition-all ${
          ...
          <button
            ...
            style={activeTab === "print_preview" ? { backgroundColor: theme.primaryColor, color: "#ffffff" } : {}}
            className={`font-bold text-xs py-2 px-3.5 rounded-xl flex items-center gap-2 cursor-pointer transition-all ${
      ...
      {/* 5 FINANCIAL / OPERATIONAL SUMMARY CARDS */}
      ...
      {activeTab === "records" && (
        <div className="space-y-4 animate-fadeIn">
          {/* Controls Bar */}
          <div 
            className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 rounded-2xl border shadow-2xs"
            style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}
          >
            <div className="flex items-center gap-3 w-full sm:w-auto flex-1">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 bg-white"
                />
              </div>
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="px-3 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-800 bg-white focus:outline-none focus:ring-2"
              >
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <button
                ...
                style={{ backgroundColor: theme.primaryColor }}
                className="flex items-center gap-2 px-4 py-2.5 hover:opacity-90 text-white rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer"
              >
          ...
          >
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left border-collapse min-w-[850px]">
                <thead>
                  <tr 
                    className="border-b text-xs font-extrabold text-slate-800 uppercase tracking-wider"
                  ...
                  {filteredServices.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="py-12 text-center text-slate-400">
                        <Laptop className="w-10 h-10 mx-auto mb-2 text-slate-300 opacity-60" />
                        ...
                        <React.Fragment key={record.id}>
                          <tr className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="py-3.5 px-4">
                              <div className="font-bold text-slate-900">{record.serviceNo}</div>
                              ...
                              <div className="relative inline-block text-left">
                                ...
                                {openStatusDropdownId === record.id && (
                                  <>
                                    ...
                                    />
                                    <div className="absolute left-0 mt-1 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-fadeIn">
                                      <div className="px-3 py-1.5 border-b border-slate-100 text-[11px] font-extrabold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                                        ...
                                        <span className="text-slate-800 font-mono font-bold text-[10px]">{record.serviceNo}</span>
                                      </div>
                                      ...
                                      <div className="pt-1.5 mt-1 border-t border-slate-100 px-2 space-y-0.5">
                                        ...
                                        <button
                                          ...
                                          className="w-full text-left px-2 py-1.5 rounded-lg text-xs font-bold text-slate-800 hover:bg-slate-100 flex items-center gap-2 transition-colors cursor-pointer"
                                        >
                            ...
                            <td className="py-3.5 px-4 text-center whitespace-nowrap">
                              <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                ...
                                <button
                                  ...
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg text-xs font-semibold transition-all cursor-pointer shadow-2xs"
                          ...
                          {/* Master-Detail Expanded Row */}
                          {isExpanded && (
                            <tr className="bg-slate-50/70 border-b border-slate-200">
                              <td colSpan={9} className="p-4 sm:p-5">
                                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs space-y-4">
                                  {/* Header info in detail card */}
                                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs pb-3 border-b border-slate-100">
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Seri No (S/N):</span>
                                      <span className="font-mono font-bold text-slate-800">{record.serialNumber || "Belirtilmemiş"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Cihaz Parola / PIN:</span>
                                      <span className="font-mono font-bold text-slate-800">{record.devicePasswordPin || "Yok / Verilmedi"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Teknisyen:</span>
                                      <span className="font-bold text-slate-800">{record.technicianName || "Laboratuvar Ekibi"}</span>
                                    </div>
                                    <div>
                                      <span className="text-slate-400 block text-[11px] font-medium">Tahmini Bitiş:</span>
                                      <span className="font-semibold text-slate-800">{record.estimatedCompletionDate || "Belirtilmemiş"}</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
                                      <span className="font-bold text-slate-700 block mb-1">Teslim Alınan Aksesuarlar / Parçalar:</span>
                                      <p className="text-slate-600">{record.deviceAccessories || "Yalnızca Cihaz (Adaptör / Çanta vb. teslim alınmadı)"}</p>
                                    </div>
                                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-200">
                                      <span className="font-bold text-slate-700 block mb-1">Veri Yedekleme & Güvenlik:</span>
                                      <p className="text-slate-600">
                                        {record.dataBackupStatus === "backup_taken"
                                          ? "✅ Müşteri verileri servis girişinde harici ortama yedeklenmiştir."
                                          : "⚠️ Cihaz yedeksiz teslim alınmıştır / müşteri veri silinmesini kabul etmiştir."}
                                      </p>
                                    </div>
                                  </div>

                                  {/* Technician Diagnosis & Internal Notes */}
                                  {(record.technicianDiagnosis || record.internalNotes) && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                                      {record.technicianDiagnosis && (
                                        <div className="p-3 bg-blue-50/60 rounded-lg border border-blue-200">
                                          <span className="font-bold text-blue-900 block mb-1">Laboratuvar Teşhisi & Onarım Raporu:</span>
                                          <p className="text-blue-800">{record.technicianDiagnosis}</p>
                                        </div>
                                      )}
                                      {record.internalNotes && (
                                        <div className="p-3 bg-amber-50/60 rounded-lg border border-amber-200">
                                          <span className="font-bold text-amber-900 block mb-1">Dahili Servis Notu:</span>
                                          <p className="text-amber-800">{record.internalNotes}</p>
                                        </div>
                                      )}
                                    </div>
                                  )}
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between font-bold text-slate-800">
                                        <span className="flex items-center gap-1.5">
                                          ...
                                          Kullanılan Donanım & Parçalar ({record.parts?.length || 0})
                                        </span>
                                        <span className="font-mono text-slate-900">
                                          ₺{(record.partsTotal || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                                        </span>
                                      </div>
                                      {record.parts && record.parts.length > 0 ? (
                                        <div className="border border-slate-200 rounded-lg overflow-hidden">
                                          <table className="w-full text-left text-[11px]">
                                            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold">
                                              <tr>
                                                <th className="py-1.5 px-2.5">Parça Adı</th>
                                                <th className="py-1.5 px-2.5 text-center">Adet</th>
                                                <th className="py-1.5 px-2.5 text-right">Birim</th>
                                            ...
                                            <tbody className="divide-y divide-slate-100 text-slate-700">
                                              {record.parts.map((p) => (
                                                <tr key={p.id}>
                                                  <td className="py-1.5 px-2.5 font-medium">{p.name}</td>
                                                  <td className="py-1.5 px-2.5 text-center font-mono">{p.quantity}</td>
                                                  <td className="py-1.5 px-2.5 text-right font-mono">₺{p.unitPrice.toFixed(2)}</td>
                                                  <td className="py-1.5 px-2.5 text-right font-mono font-bold">₺{p.totalPrice.toFixed(2)}</td>
                                                </tr>
                                              ))}
                                            </tbody>
                                          </table>
                                        ...
                                        <div className="py-3 px-2 bg-slate-50 rounded-lg border border-slate-200 text-slate-400 text-center italic text-[11px]">
                                          Kullanılan donanım / yedek parça kaydı yok
                                        </div>
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between font-bold text-slate-800">
                                        <span className="flex items-center gap-1.5">
                                          ...
                                          Yapılan Laboratuvar İşçilikleri ({record.labor?.length || 0})
                                        </span>
                                        <span className="font-mono text-slate-900">
                                          ₺{(record.laborTotal || 0).toLocaleString("tr-TR", { minimumFractionDigits: 2 })}
                                        </span>
                                      </div>
                                      {record.labor && record.labor.length > 0 ? (
                                        <div className="border border-slate-200 rounded-lg overflow-hidden">
                                          <table className="w-full text-left text-[11px]">
                                            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-semibold">
                                              <tr>
                                                <th className="py-1.5 px-2.5">İşçilik / Hizmet</th>
                                                <th className="py-1.5 px-2.5 text-center">Süre/Adet</th>
                                                <th className="py-1.5 px-2.5 text-right">Birim</th>
                                                <th className="py-1.5 px-2.5 text-right">Toplam</th>
                                              </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-100 text-slate-700">
                                              {record.labor.map((l) => (
                                                <tr key={l.id}>
                                                  <td className="py-1.5 px-2.5 font-medium">{l.description}</td>
                                                  <td className="py-1.5 px-2.5 text-center font-mono">{l.hours}</td>
                                                  <td className="py-1.5 px-2.5 text-right font-mono">₺{l.hourlyRate.toFixed(2)}</td>
                                                  <td className="py-1.5 px-2.5 text-right font-mono font-bold">₺{l.totalPrice.toFixed(2)}</td>
                                                </tr>
                                              ))}
                                            </tbody>
                                          </table>
                                  ...
                                  <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                                    <div className="text-xs text-slate-500">
                                      Son Güncelleme: <span className="font-semibold text-slate-700">{record.entryDate}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <button
                                        type="button"
