...
import { Employee, PayrollRecord, LeaveRequest, AdvanceRequest, LegalDeduction, CompanySettings, Branch, Warehouse, CostProject, AssetCustody, AdditionalPaymentItem, Transaction } from "../types";
...
import { getActivePayrollParameters } from "../data/payrollParametersData";
...
export const HRManagement: React.FC<HRManagementProps> = ({
  ...
  const handleApplyBatchPayroll = (
    ...
    // Toplu bordro kayıtlarını vergilendirme/giderler bölümüne masraf/gider işlemi olarak aktar
    if (onAddTransaction && employees && employees.length > 0) {
      try {
        const [yr, mo] = (payrollMonth || "2026-07").split("-");
        const lastDay = new Date(Number(yr), Number(mo), 0).getDate();
        const expenseDate = `${payrollMonth}-${String(lastDay).padStart(2, "0")}`;
        const monthName = MONTH_NAMES[Number(mo) - 1] || mo;

        // Her güncellenen personel için gider kalemi oluştur
        Object.keys(updatedCustomizations).forEach((empId) => {
          const emp = employees.find((e) => e.id === empId);
          if (emp) {
            const calculated = calculatePayrollForEmployee(emp);
            const cost = calculated.totalEmployerCost > 0 ? calculated.totalEmployerCost : (calculated.payableNetSalary || emp.salaryAmount || 0);
            if (cost > 0) {
              const payrollTx: Transaction = {
                id: `tx_payroll_${emp.id}_${payrollMonth.replace("-", "_")}_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
                date: expenseDate,
                type: "expense",
                ...
                category: "Personel ve Maaş Giderleri",
                description: `${emp.fullName} - ${monthName} ${yr} Bordro & Personel Gideri (Net: ${formatTRY(calculated.payableNetSalary)}, SGK/Vergi: ${formatTRY(calculated.totalEmployerCost - calculated.payableNetSalary)})`,
                documentNo: `BDR-${payrollMonth.replace("-", "")}-${emp.id.slice(-4).toUpperCase()}`,
                subtotal: Math.round(cost * 100) / 100,
                totalVat: 0,
              };
              onAddTransaction(payrollTx);
            }
          }
        });
      } catch (err) {
        console.error("Toplu bordro masraf aktarımı hatası:", err);
      }
    }

    showToast(
      `🎉 ${processedCount} personelin ${payrollMonth} dönemi bordrosu kaydedildi ve Vergilendirme / Giderler bölümüne masraf olarak aktarıldı! Toplam Net: ${formatTRY(totalNet)} • İşveren Maliyeti: ${formatTRY(totalCost)}`
    );
  };
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        triggerFormErrorNotification("Lütfen 5MB'tan küçük bir fotoğraf seçin.", "Personel Formu Dosya Boyutu Hatası", "Fotoğraf");
        return;
      }
  ...
  const calculatePayrollForEmployee = (emp: Employee): PayrollRecord => {
    ...
    const activeParams = getActivePayrollParameters();
    const isRetired = custom.isRetired !== undefined ? custom.isRetired : Boolean(emp.isRetired);

    // Emekli çalışanlar için SGDP (Sosyal Güvenlik Destek Primi) oranları uygulanır
    const sgkEmployeeRate = isRetired
      ? (activeParams.sgdpEmployeeRate ?? 0.075)
      : (activeParams.sgkEmployeeRate ?? 0.14);

    const unemploymentEmployeeRate = isRetired
      ? (activeParams.sgdpUnemploymentRate ?? 0.0)
      : (activeParams.unemploymentEmployeeRate ?? 0.01);

    const stampTaxRate = activeParams.stampTaxRate ?? 0.00759;
    
    const sgkEmployerRate = isRetired
      ? (activeParams.sgdpEmployerRate ?? 0.245)
      : (activeParams.sgkEmployerDiscountedRate ?? 0.155);

    const unemploymentEmployerRate = isRetired
      ? 0.0
      : (activeParams.unemploymentEmployerRate ?? 0.02);

    const minWageTaxExemption = activeParams.minWageTaxExemption ?? 2950;

    const sgkEmployeeShare = Math.round(grossSalary * sgkEmployeeRate);
    const unemploymentEmployeeShare = Math.round(grossSalary * unemploymentEmployeeRate);
    ...
    const besDeduction = custom.besDeduction !== undefined
      ? custom.besDeduction
      : (emp.hasBes && !isRetired ? Math.round(baseGross * (activeParams.besAutoEnrollmentRate ?? 0.03)) : 0);
    ...
    const besReason = custom.besReason !== undefined
      ? custom.besReason
      : (emp.hasBes && !isRetired ? "%3 BES Otomatik Katılım" : "");
    ...
    const sgkEmployerShare = Math.round(grossSalary * sgkEmployerRate);
    const unemploymentEmployerShare = Math.round(grossSalary * unemploymentEmployerRate);
  ...
  const handleOpenEditEmployee = (emp: Employee) => {
    ...
    setNewEmpForm({
      ...
      tckn: emp.tckn || emp.tcNo || "",
      gender: emp.gender || "Erkek",
      title: emp.title || "",
      department: emp.department || "Yazılım & IT",
      ...
      endDate: emp.endDate || "",
      terminationCode: emp.terminationCode || "",
      ...
      phone: emp.phone || "",
      email: emp.email || "",
      salaryType: emp.salaryType || "net",
      salaryAmount: emp.salaryAmount || emp.salary || 50000,
      foodAllowance: emp.foodAllowance ?? 4500,
      roadAllowance: emp.roadAllowance ?? 2000,
      hasBes: emp.hasBes ?? true,
      isRetired: Boolean(emp.isRetired),
      sgkOccupationCode: emp.sgkOccupationCode || "2512.01",
      iban: emp.iban || "TR",
      bankName: emp.bankName || "Garanti BBVA",
      emergencyContact: emp.emergencyContact || "",
      emergencyPhone: emp.emergencyPhone || "",
      status: emp.status || "active",
      annualLeaveAllowance: emp.annualLeaveAllowance ?? 14,
  ...
  const handleCreateEmployeeSubmit = (e: React.FormEvent) => {
    ...
    if (editingEmployee) {
      const updated: Employee = {
        ...editingEmployee,
        tckn: newEmpForm.tckn || editingEmployee.tckn,
        tcNo: newEmpForm.tckn || editingEmployee.tckn,
        fullName: newEmpForm.fullName || editingEmployee.fullName,
        gender: newEmpForm.gender || editingEmployee.gender || "Erkek",
        title: newEmpForm.title || editingEmployee.title,
        department: newEmpForm.department || editingEmployee.department,
        startDate: newEmpForm.startDate || editingEmployee.startDate,
        ...
        photoUrl: newEmpForm.photoUrl !== undefined ? newEmpForm.photoUrl : editingEmployee.photoUrl,
        ...
        salary: Number(newEmpForm.salaryAmount) || 50000,
        ...
        isRetired: Boolean(newEmpForm.isRetired),
      ...
      setIsAddEmployeeOpen(false);
      setEditingEmployee(null);
      showToast(`✅ ${updated.fullName} personel bilgileri başarıyla güncellendi.`);
    } else {
      const created: Employee = {
        id: `emp_${Date.now()}`,
        tckn: newEmpForm.tckn || "10000000000",
        tcNo: newEmpForm.tckn || "10000000000",
      ...
      setIsAddEmployeeOpen(false);
      showToast(`🎉 ${created.fullName} başarıyla sisteme yeni personel olarak kaydedildi.`);
    }
  ...
  const handleOpenEditPayroll = (emp: Employee) => {
    ...
    setEditingPayrollForm({
      ...
      isRetired: existing.isRetired !== undefined ? existing.isRetired : Boolean(emp.isRetired),
      besDeduction: existing.besDeduction !== undefined
        ? existing.besDeduction
        : (autoLegal.besDeduction > 0 ? autoLegal.besDeduction : (emp.hasBes && !emp.isRetired ? Math.round((emp.salaryAmount * (emp.salaryType === "net" ? 1.38 : 1)) * 0.03) : 0)),
      besReason: existing.besReason ?? (emp.hasBes && !emp.isRetired ? "%3 Otomatik BES Katılımı" : ""),
  ...
  const handleSaveEditPayroll = (
    ...
    // 4.1 Bordro kaydını vergilendirme/giderler bölümüne masraf/gider işlemi olarak aktar
    if (onAddTransaction) {
      try {
        const calculatedRec = calculatePayrollForEmployee(currentEmp);
        const costAmount = calculatedRec.totalEmployerCost > 0 ? calculatedRec.totalEmployerCost : (calculatedRec.payableNetSalary || currentEmp.salaryAmount || 0);
        if (costAmount > 0) {
          const [yr, mo] = (payrollMonth || "2026-07").split("-");
          const lastDay = new Date(Number(yr), Number(mo), 0).getDate();
          const expenseDate = `${payrollMonth}-${String(lastDay).padStart(2, "0")}`;
          ...
          const payrollTx: Transaction = {
            id: `tx_payroll_${currentEmp.id}_${payrollMonth.replace("-", "_")}_${Date.now()}`,
            date: expenseDate,
            type: "expense",
            ...
            accountId: "acc_1",
            accountName: "Merkez TL Kasası",
            contactId: currentEmp.id,
            contactName: currentEmp.fullName,
            category: "Personel ve Maaş Giderleri",
            description: `${currentEmp.fullName} - ${monthName} ${yr} Bordro & Personel Gideri (Net: ${formatTRY(calculatedRec.payableNetSalary)}, SGK/Vergi: ${formatTRY(calculatedRec.totalEmployerCost - calculatedRec.payableNetSalary)})`,
            documentNo: `BDR-${payrollMonth.replace("-", "")}-${currentEmp.id.slice(-4).toUpperCase()}`,
            subtotal: Math.round(costAmount * 100) / 100,
            totalVat: 0,
          };
          onAddTransaction(payrollTx);
        }
      } catch (err) {
        console.error("Bordro masraf aktarımı hatası:", err);
      }
    }

    // 4.2 Doğrudan veya butonla hemen yazdırma istenmişse
  ...
  if (isAddEmployeeOpen) {
    ...
    return (
        <DetailPageLayout
          title={isEditMode ? `${editingEmployee?.fullName} - Personel Bilgilerini Düzenle` : "Yeni Personel Kartı Oluştur"}
          subtitle={isEditMode ? "Personelin özlük, maaş, departman ve çalışma bilgilerini güncelleyin" : "Çalışanın kişisel, özlük ve maaş bilgilerini eksiksiz girin"}
          breadcrumbs={[
            { label: "İnsan Kaynakları", onClick: handleBackToList },
            { label: isEditMode ? "Personel Düzenle" : "Yeni Personel Kartı", active: true },
          ]}
          ...
          statusBadge={
            <span className={`px-3 py-1 text-xs font-bold rounded-xl border ${
              ...
              {isEditMode ? "Düzenleme Modu" : "Yeni Kayıt"}
            </span>
          }
          headerIcon={isEditMode ? <Edit className="w-5 h-5 text-amber-600" /> : <UserPlus className="w-5 h-5 text-purple-600" />}
          actions={
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                <span>{isEditMode ? "Değişiklikleri Kaydet" : "Kaydet ve Kartı Aç"}</span>
              </button>
            </div>
          }
        >
          <div className="bg-white rounded-3xl p-4 sm:p-6 md:p-8 border border-slate-200 shadow-sm max-w-3xl mx-auto space-y-6">
            <form id="add-employee-form" onSubmit={handleCreateEmployeeSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Emeklilik ve SGDP Statüsü & BES */}
                <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 bg-blue-50/60 border border-blue-200/80 rounded-2xl">
                  <div className="flex items-start gap-3">
                    <input
                      type="checkbox"
                      id="emp-is-retired"
                      checked={Boolean(newEmpForm.isRetired)}
                      onChange={(e) => {
                        const checked = e.target.checked;
                        setNewEmpForm({
                          ...newEmpForm,
                          isRetired: checked,
                          // Emekli personeller zorunlu BES kapsamı dışındadır, emekli seçildiğinde BES'i otomatik pasife alabiliriz
                          hasBes: checked ? false : newEmpForm.hasBes,
                        });
                      }}
                      className="mt-1 w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500 cursor-pointer"
                    />
                    <div>
                      <label htmlFor="emp-is-retired" className="block text-xs font-bold text-slate-900 cursor-pointer">
                        Emekli Çalışan (SGDP Kapsamında)
                      </label>
                      <p className="text-[11px] text-slate-600 mt-0.5 leading-snug">
                        Emekli maaşı alıp çalışan personellerde İşsizlik Sigortası kesilmez (%0), SGDP (%7.5 işçi, %24.5 işveren) uygulanır ve otomatik BES muafiyeti sağlanır.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    <input
                      type="checkbox"
                      id="emp-has-bes"
                      checked={Boolean(newEmpForm.hasBes)}
                      disabled={Boolean(newEmpForm.isRetired)}
                      onChange={(e) => setNewEmpForm({ ...newEmpForm, hasBes: e.target.checked })}
                      className="mt-1 w-4 h-4 text-purple-600 rounded border-slate-300 focus:ring-purple-500 cursor-pointer disabled:opacity-50"
                    />
                    <div>
                      <label htmlFor="emp-has-bes" className={`block text-xs font-bold cursor-pointer ${newEmpForm.isRetired ? "text-slate-400" : "text-slate-900"}`}>
                        Bireysel Emeklilik Sistemi (BES %3)
                      </label>
                      <p className="text-[11px] text-slate-600 mt-0.5 leading-snug">
                        {newEmpForm.isRetired
                          ? "Emekli çalışanlar yasal olarak zorunlu BES kesintisinden muaftır."
                          : "4632 sayılı Kanun uyarınca aylık prime esas kazancın %3'ü oranında otomatik BES fon kesintisi yapılır."}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-6 py-2 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs transition-all shadow-md cursor-pointer flex items-center gap-1.5"
                >
                  {isEditMode ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                  <span>{isEditMode ? "Değişiklikleri Kaydet" : "Kaydet ve Kartı Aç"}</span>
                </button>
          ...
          <div className="bg-white rounded-3xl w-full p-4 sm:p-6 md:p-8 border border-purple-100 shadow-sm space-y-6 max-w-7xl mx-auto">
            ...
            <form onSubmit={handleSaveEditPayroll} className="space-y-4">
              ...
              {(() => {
                ...
                const isEmpRetired = editingPayrollForm.isRetired !== undefined ? editingPayrollForm.isRetired : Boolean(editingPayrollEmp?.isRetired);
                const sgkEmpRate = isEmpRetired ? 0.075 : 0.14;
                const unempEmpRate = isEmpRetired ? 0 : 0.01;

                const sgkEmp = Math.round(grossSal * sgkEmpRate);
                const unempEmp = Math.round(grossSal * unempEmpRate);
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5">
                        {isEmpRetired && (
                          <span className="text-[10px] bg-blue-500/40 text-blue-200 border border-blue-400/50 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                            🏖️ Emekli Personel (SGDP: %7.5 İşçi / %0 İşsizlik / BES Muaf)
                          </span>
                        )}
                        {lvsDays > 0 && (
          ...
          actions={
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-xs cursor-pointer shadow-xs transition-all flex items-center gap-1.5"
                title="Personel Kartını ve Bilgilerini Düzenle"
              >
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 sm:space-y-7 max-w-7xl mx-auto">
      ...
      <ModuleEntranceHeader
        badge="İnsan Kaynakları & Bordro"
        badgeIcon={<UserCheck className="w-2.5 h-2.5 text-[#0f6bae]" />}
        title="Personel, Bordro ve Özlük Takibi"
        description="Çalışan özlük dosyaları, kanuni SGK bordro matrahları, yıllık izin hakkı ve masraf/avans taleplerini tek ekrandan yönetin."
        actions={
          <>
            <button
              type="button"
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-800 transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs shrink-0"
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={handleOpenCreateEmployee}
                  className="px-3.5 py-2 bg-gradient-to-r from-purple-700 to-indigo-700 hover:from-purple-800 hover:to-indigo-800 text-white font-bold rounded-xl text-xs shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95 shrink-0"
            ...
            <table className="w-full text-left text-xs border-separate border-spacing-y-2.5 min-w-[720px]">
              ...
              <tbody>
                {filteredEmployees.map((emp) => (
                  <tr
                    key={emp.id}
                    className="bg-white hover:bg-gradient-to-r hover:from-blue-50/90 hover:via-sky-50/60 hover:to-blue-50/90 hover:shadow-[0_1px_3px_rgba(15,23,42,0.04),0_1px_2px_rgba(15,23,42,0.02)] transition-all duration-200 group rounded-xl relative z-0 hover:z-10 cursor-pointer"
                  >
                    <td className="py-3 px-3 rounded-l-xl border-y border-l border-purple-200/50 group-hover:border-blue-300 group-hover:bg-blue-50/30 transition-all">
                      <div>
                        <div className="font-extrabold text-slate-900 group-hover:text-blue-950 text-sm flex items-center gap-1.5 wrap flex-wrap">
                          ...
                          {emp.isRetired && (
                            <span className="text-[10px] font-extrabold bg-blue-50 text-blue-800 border border-blue-200 px-1.5 py-0.5 rounded-full whitespace-nowrap inline-flex items-center gap-1">
                              <span>🏖️</span>
                    ...
                    <td className="py-3 px-3 border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-mono tabular-nums font-tabular-num-md">
                      <div className="font-bold text-slate-900 font-mono tabular-nums font-tabular-num-md">
                        {formatTRY(emp.salaryAmount)}
                        <span className="text-[10px] text-slate-500 uppercase ml-1 font-sans">
                          ({emp.salaryType})
                        </span>
                      </div>
                      <div className="text-[11px] text-slate-500 font-mono tabular-nums font-tabular-num-md">
                    ...
                    <td className="py-2.5 sm:py-3 px-2 sm:px-3 rounded-r-xl border-y border-r border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1 sm:gap-1.5">
                        <button
                          ...
                          className="bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200/80 font-bold p-1.5 sm:px-2.5 sm:py-1.5 rounded-lg sm:rounded-xl text-xs transition-all inline-flex items-center gap-1 cursor-pointer shadow-2xs active:scale-95 shrink-0"
                          title={`${emp.fullName} bilgilerini ve personel kartını düzenle`}
                          aria-label="Personeli Düzenle"
                        >
                          <Edit className="w-3.5 h-3.5 text-amber-700 shrink-0" />
                          <span className="hidden sm:inline lg:hidden xl:inline">Düzenle</span>
                        </button>
                        <button
                          type="button"
                ...
                <tbody>
                  {payrollRecords.map((rec) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-3 px-3 rounded-l-xl border-y border-l border-purple-200/50 group-hover:border-blue-300 group-hover:bg-blue-50/30 transition-all">
                          <div className="font-bold text-slate-900 group-hover:text-blue-950 text-sm flex items-center gap-1.5">
                        ...
                        <td className="py-3 px-3 border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-mono tabular-nums font-tabular-num-md font-bold text-slate-800">
                          {formatTRY(rec.grossSalary)}
                          {Boolean(rec.bonusAmount || rec.overtimePay) && (
                            <div className="space-y-0.5 mt-0.5">
                              {Boolean(rec.bonusAmount) && (
                                <div className="text-[10px] text-emerald-600 font-mono tabular-nums font-tabular-num-md font-medium">
                                  +{formatTRY(rec.bonusAmount)} Prim
                                </div>
                              )}
                              {Boolean(rec.overtimePay) && (
                                <div className="text-[10px] text-purple-700 font-mono tabular-nums font-tabular-num-md font-bold bg-purple-50 px-1 py-0.5 rounded border border-purple-200/70" title={`Fazla Mesai: ${((rec.overtimeNormalHours || 0) + (rec.overtimeWeekendHours || 0) + (rec.overtimeHolidayHours || 0))} Saat / ${rec.overtimeHolidayDays || 0} Gün`}>
                        ...
                        <td className="py-3 px-3 border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-mono tabular-nums font-tabular-num-md text-slate-600">
                          <div>{formatTRY(totalLegalDeductions)}</div>
                          <div className="text-[10px] text-slate-400 font-mono tabular-nums font-tabular-num-md">SGK: {formatTRY(rec.sgkEmployeeShare)} · GV: {formatTRY(rec.incomeTax)}</div>
                        </td>
                        ...
                        <td className="py-3 px-3 border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-black text-emerald-800 text-sm font-mono tabular-nums font-tabular-num-md">
                          {formatTRY(rec.payableNetSalary)}
                        </td>

                        <td className="py-3 px-3 border-y border-purple-200/50 group-hover:border-purple-300 group-hover:bg-purple-50/30 transition-all font-black text-purple-950 text-sm font-mono tabular-nums font-tabular-num-md">
                      ...
                      <tbody>
                        {employeeLeaveDetails.map((d) => (
                          <tr
                            ...
                            className="bg-white hover:bg-gradient-to-r hover:from-blue-50/90 hover:via-sky-50/60 hover:to-blue-50/90 hover:shadow-[0_1px_3px_rgba(15,23,42,0.04),0_1px_2px_rgba(15,23,42,0.02)] transition-all duration-200 group rounded-xl relative z-0 hover:z-10 cursor-pointer"
                          >
                            <td className="py-3 px-3 rounded-l-xl border-y border-l border-purple-200/50 group-hover:border-blue-300 group-hover:bg-blue-50/30 transition-all font-bold text-slate-900 group-hover:text-blue-950">
                              <div>
              ...
              <table className="w-full text-left text-xs border-separate border-spacing-y-2.5 min-w-[800px]">
                ...
                {leaveRequests.map((req) => {
                  ...
                  <tr
                    key={req.id}
                    id={`leave-row-${req.id}`}
                    className={`transition-all duration-200 group rounded-xl relative cursor-pointer ${
                      isHighlighted
                        ? "bg-purple-100 ring-2 ring-purple-600 shadow-xl z-20 scale-[1.006]"
                        : "bg-white hover:bg-gradient-to-r hover:from-blue-50/90 hover:via-sky-50/60 hover:to-blue-50/90 hover:shadow-[0_1px_3px_rgba(15,23,42,0.04),0_1px_2px_rgba(15,23,42,0.02)] z-0 hover:z-10"
                    }`}
                  >
                    <td className={`py-3 px-3 rounded-l-xl border-y border-l transition-all font-bold ${
                      isHighlighted
                        ...
                        : "border-purple-200/50 group-hover:border-blue-300 group-hover:bg-blue-50/30 text-slate-900 group-hover:text-blue-950"
                    }`}>
                      <div className="flex items-center gap-2 flex-wrap">
